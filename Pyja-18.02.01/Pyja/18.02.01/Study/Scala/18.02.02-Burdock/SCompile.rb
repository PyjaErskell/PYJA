...
module DBody
  def self.dp_it
    ...
    @__dau_src_fn = GC_ARGV[0]
    GC_LOG .info "Scala source file => #{@__dau_src_fn}"
    raise RuntimeError, 'Scala source file not exist !!!' unless gf_xi @__dau_src_fn
    __dap_compile
    __dap_make_jar
  ensure
    gp_rp @__dau_cls_o_pn
    QTimer .single_shot 0, *gf_qt_slot { GC_QAPP .quit }
  end
private
  def self.__dap_compile
    gp_log_header 'Compiling source file ...', 30
    pu_java_x_fn = gf_os_env :SC_J8_X_FN
    GC_LOG .info "Java executable file => #{pu_java_x_fn}"
    pu_cp = [
      ( gf_os_env :SC_QTJ_JAR_FN ),
    ...
    pu_jar_fns = __daf_jar_fns
    gp_log_array  'JAR files for compiling scala source', pu_jar_fns .map { |bx2_jar_fn| gf_to_prs bx2_jar_fn }
    GC_LOG .info "Compiling #{ gf_bn @__dau_src_fn } ..."
    Thread.new do
      system pu_java_x_fn, '-Xmx256M', '-Xms32M', "-Xbootclasspath/a:#{pu_cp}", '-classpath', '', "-Dscala.boot.class.path=#{pu_cp}", 'scala.tools.nsc.Main', '-feature', '-target:jvm-1.8', '-classpath', pu_jar_fns.join(GC_PASA), '-d', @__dau_cls_o_pn, @__dau_src_fn
      raise RuntimeError, 'Error during compiling !!!' unless $?.exitstatus == 0
    end .join
  end
  def self.__dap_make_jar
    gp_log_header 'Making jar file ...', 30
    pu_out_jar_pn = gf_pj GC_MILO_PN, 'jar'
    gp_mp pu_out_jar_pn
    pu_out_jar_fn = gf_pj pu_out_jar_pn, "#{ gf_jn @__dau_src_fn }.jar"
    GC_LOG .info "Output JAR file => #{ gf_to_mps pu_out_jar_fn }"
    pu_jar_x_fn = gf_os_env :SC_J8R_X_FN
    GC_LOG .info "Jar executable file => #{pu_jar_x_fn}"
    GC_LOG .info 'Making jar file ...'
    Thread.new do
      system pu_jar_x_fn, '-cvf', pu_out_jar_fn, '-C', @__dau_cls_o_pn, '.'
      raise RuntimeError, 'Error during making jar !!!' unless $?.exitstatus == 0
    end .join
  end
  def self.__daf_jar_fns
    fu_fns = []
    fv_extract = false
    File.readlines(@__dau_src_fn).each do |bx2_ln|
      unless fv_extract
        fv_extract = true if bx2_ln =~ %r{^\s*//\s*jar\*\s*$}
      else
        if bx2_ln =~ %r{^\s*//\s*\*raj\s*$}
          fv_extract = false
        else
          unless bx2_ln =~ %r{^\s*//}
            %r{\s*gf_pj\s*\(\s*(?<bu5_args>.+)\s*\)\s*,\s*$}i =~ bx2_ln
            fu_fns << ( eval "gf_pj #{bu5_args}" )
          end
        end
      end
    end
