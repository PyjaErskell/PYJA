...
class WAtQMain ( CjAt, TjUtil, TgWai ) :
  ...
  def receive ( self, x_letter, x_atr_sender ) :
    ...
    elif nu_l_jn == 'LResult' :
      bu2_nr_of_words = x_letter .lu_it ()
      self.wn_log_info ( f'Total number of words => {bu2_nr_of_words:,d}' )
      gp_log_lines ( self.wn_log_info, gf_box ( [ f'Elapsed { jf_elapsed (self.wu_st) } ...' ] ) )
  def wn_do_it (self) :
    nu_fn = gf_pj ( GC_MILO_PN, f"{ gf_jn (GC_SCRIPT_FN) }.scala" )
    JC_LOG .info ( f'File name for processing => { gf_to_mps (nu_fn) }' )
    nu_atr = jf_sc_mk_atr ( jf_jcls ('CAtFileWordCounter'), [ self .cm_self (), nu_fn ], ':c' )
    self.wu_st = datetime.now ()
    self .cn_tell ( nu_atr, CjSymbol ('LDoIt') )
