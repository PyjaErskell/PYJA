...
XBorderPane = jf_jcls ('javafx.scene.layout.BorderPane')
XColor      = jf_jcls ('javafx.scene.paint.Color')
XEllipse    = jf_jcls ('javafx.scene.shape.Ellipse')
XGridPane   = jf_jcls ('javafx.scene.layout.GridPane')
XLabel      = jf_jcls ('javafx.scene.control.Label')
XLine       = jf_jcls ('javafx.scene.shape.Line')
XPane       = jf_jcls ('javafx.scene.layout.Pane')
XScene      = jf_jcls ('javafx.scene.Scene')

class WAtMain ( CjAt, TjUtil, TgWai ) :
  def __init__ (self) : super () .__init__ ()
  def receive ( self, x_letter, x_atr_sender ) :
    nu_ln = x_letter.java_name
    JC_LOG .trace ( self .tm_wai ( f'Received {nu_ln}' ) )
  def preStart (self) :
    JC_LOG .debug ( self .tm_wai ('Entered ...') )
    self .wn_init ()
  def wn_init (self) :
    JC_LOG .info ( self.tm_wai ('Initializing ...') )
    self.wv_game_over = False
    self.wv_nr_filled = 0
    self.wv_whose_turn = 'X' # 'X' or 'O'
    self.wu_cells = {}
    def nf2_gp (it) :
      for bu2_i in range (3) :
        for bu3_j in range (3) :
          bu3_cell = WCell (self)
          it .add ( bu3_cell .wu_xo, bu3_j, bu2_i )
          self.wu_cells [ bu2_i, bu3_j ] = bu3_cell
      return it
    self.wu_gp = nf2_gp ( XGridPane () )
    self.wu_lb_status = XLabel ( "X's turn to play" )
    def nf2_root (it) :
      it .setCenter (self.wu_gp)
      it .setBottom (self.wu_lb_status)
      return it
    self.wu_root = nf2_root ( XBorderPane () )
    def nf2_sg (it) :
      ...
      self .tn_fx_set_eh ( it, 'onShowing', 'wn_sg_on_showing' )
      self .tn_fx_set_eh ( it, 'onShown', 'wn_sg_on_shown' )
      self .tn_fx_set_eh ( it, 'onCloseRequest', 'wn_sg_on_close_request' )
      return it
    self.wu_sg = nf2_sg (JC_FX_STAGE)
    self.wu_sg .show ()
  def wm_is_full (self) : return self.wv_nr_filled >= 9 # > should never happen
  def wm_has_won ( self, x_token ) :
    def mf2_c ( x2_i, x2_j ) : return self.wu_cells [ x2_i, x2_j ] .wm_token () == x_token
    for bu2_i in range (3) :
      if mf2_c ( bu2_i, 0 ) and mf2_c ( bu2_i, 1 ) and mf2_c ( bu2_i, 2 ) : return True
    for bu2_j in range (3) :
      if mf2_c ( 0, bu2_j ) and mf2_c ( 1, bu2_j ) and mf2_c ( 2, bu2_j ) : return True
    if mf2_c ( 0, 0 ) and mf2_c ( 1, 1 ) and mf2_c ( 2, 2 ) : return True
    if mf2_c ( 0, 2 ) and mf2_c ( 1, 1 ) and mf2_c ( 2, 0 ) : return True
    return False
  def wn_center_on_screen (self) :
    nu_cp = QDesktopWidget () .availableGeometry () .center () # center point
    self.wu_sg .setX ( nu_cp .x () - self.wu_sg .getWidth () / 2 )
    self.wu_sg .setY ( nu_cp .y () - self.wu_sg .getHeight () / 2 )
  def wn_sg_on_showing ( self, x_ev ) :
    JC_LOG .debug ( 'Stage showing ...' )
    self .wn_center_on_screen ()
  def wn_sg_on_shown ( self, x_ev ) :
    JC_LOG .info ( 'Stage shown ...' )
  def wn_sg_on_close_request ( self, x_ev ) :
    JC_LOG .info ( self .tm_wai ( 'About to quit ...' ) )
    jp_request_exit (GC_EC_SUCCESS)

class WCell ( CgQo, TjUtil, TgWai ) :
  def __init__ ( self, x_parent ) :
    super () .__init__ ()
    self.wu_parent = x_parent
    self .wn_init ()
  def wn_init (self) :
    JC_LOG .info ( self.tm_wai ('Initializing ...') )
    self .wv_token = ' ' # one of blank, X, or O
    def nf2_xo (it) :
      it .setStyle ( "-fx-border-color: PeachPuff" )
      it .setPrefSize ( 100, 100 )
      self .tn_fx_set_eh ( it, 'onMouseClicked', 'wn_xo_on_mouse_clicked' )
      return it
    self.wu_xo = nf2_xo ( XPane () )
  def wm_token (self) : return self.wv_token
  def wn_set_token ( self, x_whose_turn ) :
    if x_whose_turn == 'X' : self .wn_draw_x ()
    else : self .wn_draw_o ()
    self .wv_token = x_whose_turn
    self.wu_parent.wv_nr_filled += 1
  def wn_draw_x (self) :
    nu_xo = self.wu_xo
    nu_w, nu_h = nu_xo .getWidth (), nu_xo .getHeight ()
    nu_ln1 = XLine ( 10, 10, nu_w - 10, nu_h - 10 )
    nu_ln2 = XLine ( 10, nu_h - 10, nu_w - 10, 10 )
    nu_xo .getChildren () .addAll ( [ nu_ln1, nu_ln2 ] )
  def wn_draw_o (self) :
    nu_xo = self.wu_xo
    nu_w, nu_h = nu_xo .getWidth (), nu_xo .getHeight ()
    def nf2_ellipse (it) :
      it .setStroke ( XColor.BLACK )
      it .setFill ( XColor.WHITE )
      return it
    nu_ellipse = nf2_ellipse ( XEllipse ( nu_w / 2, nu_h / 2, nu_w / 2 - 10, nu_h / 2 - 10 ) )
    nu_xo .getChildren () .add (nu_ellipse)
  def wn_xo_on_mouse_clicked ( self, x_ev ) :
    JC_LOG .trace ( self .tm_wai ('Begin ...') )
    nu_p = self.wu_parent
    nl_msg = ''
    if not nu_p.wv_game_over :
      self .wn_set_token (nu_p.wv_whose_turn)
      if nu_p .wm_has_won (nu_p.wv_whose_turn) :
        nu_p.wv_game_over = True
        nl_msg = f'{nu_p.wv_whose_turn} won !!! The game is over'
      elif nu_p .wm_is_full () :
        nu_p.wv_game_over = True
        nl_msg = 'Draw !!! The game is over'
      else :
        nu_p.wv_whose_turn = 'O' if nu_p.wv_whose_turn == 'X' else 'X'
        nl_msg = f"{nu_p.wv_whose_turn}'s turn"
      nu_p.wu_lb_status .setText (nl_msg)
