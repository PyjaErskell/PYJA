...
case class LLine ( lu_line_nr : Integer, lu_str : String )
case class LResult ( lu_it : Integer )
case class LWords ( lu_it : Integer )

class CAtLineWordCounter ( cu_atr_main : ActorRef ) extends Actor {
  def receive = {
    case LLine ( bu2_line_nr, bu2_str ) => {
      cu_atr_main ! LDisplay ( CLogLevel.INFO, s"${ "%3d" format (bu2_line_nr) } : ${bu2_str}" )
      val bu2_words = bu2_str .split (" ") .length
      sender ! LWords (bu2_words)
    }
    case bu2_invalid => cu_atr_main ! LDisplay ( CLogLevel.ERROR, s"Message ${bu2_invalid} not recognized" )
  }
}

class CAtFileWordCounter ( cu_atr_main : ActorRef, cu_fn : String ) extends Actor {
  private var cv_running = false
  private var cv_total_lines = 0
  private var cv_lines_processed = 0
  private var cv_result = 0
  def receive = {
    case bu2_it @ 'LDoIt => {
      if (cv_running) {
        cu_atr_main ! LDisplay ( CLogLevel.WARN, s"Duplicate ${bu2_it} message received" )
      } else {
        cv_running = true
        import scala.io.Source._
        fromFile (cu_fn) .getLines .foreach { bx4_str =>
          cv_total_lines += 1
          context .actorOf ( Props ( new CAtLineWordCounter (cu_atr_main) ) ) ! LLine ( cv_total_lines, bx4_str )
        }
      }
    }
    case LWords (bu2_words) => {
      cv_result += bu2_words
      cv_lines_processed += 1
      if ( cv_lines_processed == cv_total_lines ) {
        cu_atr_main ! LResult (cv_result) // provide result to process invoker
      }
    }
    case bu2_invalid => GC_LOG .warn ( s"Message ${bu2_invalid} not recognized !!!" )
  }
}
