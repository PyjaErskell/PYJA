...
XListView  = jf_jcls ('javafx.scene.control.ListView')
XPos       = jf_jcls ('javafx.geometry.Pos')
XScene     = jf_jcls ('javafx.scene.Scene')
XStackPane = jf_jcls ('javafx.scene.layout.StackPane')
...
class WAtXMain ( CjAt, TjUtil, TgWai ) :
  ...
  def wn_init (self) :
    ...
    self.wu_lv = XListView ()
    def nf2_root (it) :
      it .setAlignment (XPos.CENTER)
      it .setPadding ( XInsets (3) )
      it .getChildren () .addAll ( [self.wu_lv] )
      return it
    self.wu_root = nf2_root ( XStackPane () )
    def nf2_stage (it) :
  ...
  def receive ( self, x_letter, x_atr_sender ) :
    ...
    if ( nu_ln == 'LDisplay' ) : self.wu_lv .getItems () .add ( x_letter .lu_msg () )
  ...
  def wn_stage_on_shown ( self, x_ev ) :
    ...
    nv_sn = 0
    def np2_greet ( x2_atr, x2_who ) :
      nonlocal nv_sn
      nv_sn += 1
      self .cn_tell ( x2_atr, jf_jcls ('LWhoToGreet') ( f'({nv_sn:03d}) {x2_who}' ) )
      self .cn_tell ( x2_atr, CjSymbol ('LGreet') )
    self.wu_atr_printer  = jf_sc_mk_atr ( jf_jcls ('CAtPrinter'), [ self .cm_self () ], ':c' )
    self.wu_atr_hello    = jf_sc_mk_atr ( jf_jcls ('CAtGreeter'), [ '-----HELLOHELLOHELLO-----', self.wu_atr_printer ], ':a' )
    self.wu_atr_howdy    = jf_sc_mk_atr ( jf_jcls ('CAtGreeter'), [ 'HOWDY', self.wu_atr_printer ], ':a' )
    self.wu_atr_good_day = jf_sc_mk_atr ( jf_jcls ('CAtGreeter'), [ 'HAVE*A*GOOD*DAY', self.wu_atr_printer ], ':a' )
    for bu2_who in [ 'CPython', 'PyQt', 'Jep' ] : np2_greet ( self.wu_atr_hello, bu2_who )
    for bu2_who in [ 'JavaFX', 'Groovy', 'Scala', 'Akka', 'Tcl', 'Play', 'Jep' ] : np2_greet ( self.wu_atr_howdy, bu2_who )
    for bu2_who in [
      'Programming with CPython, Scala, Groovy, Java, JRuby ...',
      'You can use PyQt and JavaFX for GUI',
      'You can use enormous libraries from CPython, Java, JRuby ...',
