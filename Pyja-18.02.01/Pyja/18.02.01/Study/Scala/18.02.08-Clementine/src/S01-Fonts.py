...
GC_GR .eval ('''
  Object.metaClass.__gau_jep = Global .GC_JEP ()
...
CgScalaSymbol = gf_jcls ('scala.Symbol')
...
class WAtQtMain (CgAt) :
  ...
  def receive ( self, x_letter, x_sender_atr ) :
    if ( x_letter.getClass () == CgScalaSymbol ) :
  ...
  def wn_change_font ( self, x_tell = True ) :
    ...
    if x_tell and self.wu_atr_fx_main is not ... : self .tell ( self.wu_atr_fx_main, CgScalaSymbol ('LNextFont') )
