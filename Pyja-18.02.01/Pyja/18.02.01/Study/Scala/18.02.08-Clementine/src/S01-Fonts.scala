...
object Global {
  ...
  val GC_JEP = new jep.Jep ( false, "." )
  private val __gau_py_fn = gf_pj ( GC_MILO_PN, "src", s"${ gf_jn (GC_THIS_JAR_FN) }.py" )
  GC_JEP .set ( "GC_SCRIPT_FN", __gau_py_fn )
  GC_JEP .runScript (__gau_py_fn)
  
  def gf_py_ver = GC_JEP .getValue ( "platform .python_version ()" ) 
  def gf_os_available_memory = GC_JEP .getValue ( "gf_os_available_memory ()" ) .asInstanceOf [Long]
  def gf_qapp_exec = GC_JEP .getValue ( "GC_QAPP .exec_ ()" ) .asInstanceOf [Long] .asInstanceOf [Int]

  lazy val GC_TOTAL_CPU = GC_JEP .getValue ("GC_TOTAL_CPU") .asInstanceOf [Long]
  lazy val GC_TOTAL_MEMORY = GC_JEP .getValue ("GC_TOTAL_MEMORY") .asInstanceOf [Long]
  lazy val GC_HOST_NM = GC_JEP .getValue ("GC_HOST_NM") .asInstanceOf [String]
  lazy val GC_CUSR = gf_sys_props ("user.name")
  lazy val GC_THIS_PID = GC_JEP .getValue ("GC_THIS_PID") .asInstanceOf [Long]
  lazy val GC_APP_NM = GC_JEP .getValue ("GC_APP_NM") .asInstanceOf [String]
  lazy val GC_CMD = GC_JEP .getValue ("GC_CMD") .asInstanceOf [java.util.ArrayList[String]] .asScala
  lazy val GC_ARGV = GC_JEP .getValue ("GC_ARGV") .asInstanceOf [java.util.ArrayList[String]] .asScala
  ...
  private object __DgExit {
    ...
    def df_was_lgcx_processed = __dav_was_lgcx_processed
    private def __dap_before_exit {
      val pu_ec = __dav_cx.lu_ec
      if ( pu_ec != GC_EC_SUCCESS ) {
        gp_log_seq () {"CLASSPATH"} {
          ClassLoader .getSystemClassLoader .asInstanceOf [java.net.URLClassLoader] .getURLs .sortBy (_.getPath) .map  { it => gf_to_prs (it.getPath) }
        }
      }
  ...
  def gf_was_lgcx_processed = __DgExit.df_was_lgcx_processed
  ...
  }; private class __CgFxApp extends javafx.application.Application {
    override def start ( x_stage : javafx.stage.Stage ) { __CgFxApp.__casp_start ( this, x_stage) }
  }
...
object DRun {
  ...
  private def __dap_begin {
    ...
    GC_LOG .debug ( s"Python version => ${gf_py_ver}" )
    GC_LOG .debug ( s"Total CPU => ${GC_TOTAL_CPU}" )
    GC_LOG .debug ( s"Total memory => ${ "%,d" .format (GC_TOTAL_MEMORY) } bytes" )
    GC_LOG .debug ( s"Available memory => ${ "%,d" .format (gf_os_available_memory) } bytes" )
    ...
    if ( GC_CMD.length > 0 ) gp_log_seq (GC_LOG.debug) {"Command"} {GC_CMD}
  ...
  def dp_it {
    try {
      __dap_begin
      DBody .dp_it
      __dav_ec = gf_qapp_exec
    } catch {
      case bu2_ex : Exception =>
        __dav_ec = GC_EC_ERROR
        __dav_ex = bu2_ex
    } finally {
      if ( ! gf_was_lgcx_processed ) {
        GC_LOG .info ( "PyQt Application exited" )
...
class WAtFxMain ( wu_stg : javafx.stage.Stage, wu_atr_qt_main : akka.actor.ActorRef ) extends akka.actor.Actor {
  ...
  def wn_init {
    ...
    gf_tap (wu_stg) { it =>
      ...
      it .setOnCloseRequest { _ =>
        ...
        wu_atr_qt_main ! 'LQuit
      }
      wn_move_center
      it .show
    }
    wu_atr_qt_main ! 'LNextFont
  }
...
object DBody {
  def dp_it {
    ...
    val pu_atr_qt_main = GC_JEP .invoke ( "gf_mk_atr", "WAtQtMain", List () .asJava, ":c" ) .asInstanceOf [akka.actor.ActorRef]
    gp_fx_start { ( bx2_app, bx2_stage ) => gf_mk_atr () ( classOf [WAtFxMain], Seq ( bx2_stage, pu_atr_qt_main ), ":c" ) }
  }
}
