...
class WMain
  ...
  def __wan_init
    ...
    jy_ge <<-NASH_EOS
      ...
      __sap_fx_start = { final x_app, final x_stage ->
        if ( ! Platform .isMac () ) {
          final bu2_msg = 'This is macOS specific sample !!!'
          GC_LOG .info bu2_msg
          new Alert (Alert.AlertType.INFORMATION) .with {
            setTitle GC_APP_NM
            setHeaderText bu2_msg
            setContentText 'You should run this sample in macOS'
            showAndWait ()
          }
        ...
        final pu_bn = new Button () .with {
          text = "(${pv_cnt+1}) Hello World"
          onAction = { bu3_ev ->
            pv_cnt ++
            GC_LOG .info text
            gp_bn pu_bi, 'wn_bn', pv_cnt, { x4_text -> gp_xr { text = x4_text } }
          }
          it
        }
        final pu_root = new StackPane () .with {
          children .add pu_bn
          it
        }
        pu_stage .with {
          ...
          onCloseRequest = { gp_bn pu_bi, 'wn_sw_quit' }
          show ()
        }
      }
  ...
  def wn_bn x_cnt, xp_it
    jy_gc xp_it, "(#{ x_cnt.intValue + 1 }) Hello World from Ruby"
  end
  def wn_sw_quit
