...
class WMain < Gtk::Window
  ...
  def __wan_init
    ...
    @wu_bn = Gtk::Button.new :label => "Number : #{ ( jy_gm CjString, 'format', '%,d', @wv_no ) .toString }"
    @wu_bn .signal_connect 'clicked' do |it|
      @wv_no += Random.new.rand 5..100000000
      it .set_label "Number : #{ ( jy_gm CjString, 'format', '%,d', @wv_no ) .toString }"
      @wu_lb .set_label "Spellout : #{ @wu_rbnf.format @wv_no }"
    end
    @wu_lb = Gtk::Label.new "Spellout : #{ @wu_rbnf.format @wv_no }"
    @wu_lo = Gtk::Box.new :vertical, 0
    @wu_lo.pack_start ( Gtk::Alignment.new 0, 0, 0, 0 ), :expand => true
    @wu_lo .add @wu_bn
    @wu_lo.pack_start ( Gtk::Alignment.new 0, 0, 0, 0 ), :expand => true
    @wu_lo .add @wu_lb
    @wu_lo.pack_start ( Gtk::Alignment.new 0, 0, 0, 0 ), :expand => true
