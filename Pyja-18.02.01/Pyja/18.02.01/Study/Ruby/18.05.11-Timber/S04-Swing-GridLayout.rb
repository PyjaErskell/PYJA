...
def jy_gc x_closure, *x_args; jy_gm x_closure, 'call', *x_args; end
...
class WMain
  ...
  def __wan_init
    @wu_bi = gf_bi self
    jy_ge <<-NASH_EOS.unindent
      import javax.swing.*
      import java.awt.event.*
      import java.awt.*
      Thread .start {
        def bv2_cnt = 0
        final def bu2_yi = #{@wu_bi}
        final def bu2_fm = new JFrame () .with {
          setTitle GC_APP_NM
          setSize 400, 400
          setLayout new GridLayout ( 3, 1 )
          setLocationRelativeTo null
          addWindowListener ( [
            windowClosing : { gp_bn bu2_yi, 'wn_sw_quit' }
          ] as WindowAdapter )
          it
        }
        final def bu2_lb_header = new JLabel ( 'This is JFrame with GridLayout', JLabel.CENTER )
        final def bu2_pl_control = new JPanel ( layout : new FlowLayout () )
        final def bu2_lb_status = new JLabel ( text : '', horizontalAlignment : JLabel.CENTER, size : [ 350, 100 ] )
        final def bu2_bn_ok = new JButton ( "Click me with count ${bv2_cnt+1}" ) .with { bx3_bn ->
          bx3_bn .actionPerformed = {
            bv2_cnt ++
            gp_bn bu2_yi, 'wn_bn_ok', bx3_bn, bu2_lb_status, bv2_cnt
          }
          bu2_pl_control .add bx3_bn
          bx3_bn
        }
        [ bu2_lb_header, bu2_pl_control, bu2_lb_status ] .each { bu2_fm .add it }
        bu2_fm .setVisible true
      }
    NASH_EOS
  end
  def wn_bn_ok x_bn_ok, x_lb_status, x_cnt
    x_lb_status .setText "(#{x_cnt.intValue}) Hello Swing from Ruby"
    x_bn_ok .setText "Click me with count #{x_cnt.intValue+1}"
  end
  def wn_sw_quit
...
module DBody
  def self.dp_it
    JC_QAPP .initialize []
    jf_jcls('com.trolltech.qt.gui.QMainWindow').new.tap { |it| it .show; it .raise; it .close } if CjPlatform .isMac # for bringing widget to the front in macOS
    pu_main = WMain .new
    JC_QAPP .execStatic
  end
end
