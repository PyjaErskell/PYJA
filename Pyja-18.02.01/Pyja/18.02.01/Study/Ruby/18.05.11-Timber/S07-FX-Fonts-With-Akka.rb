...
class WAtMain < CjAt
  ...
  def wn_init x_stage
    ...
    @wu_stage = x_stage
    gp_set_const :CjButton, ( jf_jcls 'javafx.scene.control.Button' )
    gp_set_const :CjFont, ( jf_jcls 'javafx.scene.text.Font' )
    ...
    @wv_msg = ''
    @wu_bn = CjButton.new .tap { |it|
      tn_fx_set_eh it, 'onAction', 'wn_bn_on_action'
    }
    @wu_lb = CjLabel.new
    wn_change_font
  ...
  def receive x_letter, x_atr_sender
    ...
    wn_change_font if nu_java_nm == 'LNextFont'
  end
  ...
  def wn_change_font
    ...
    jy_gc "{ x_bn, x_text, x_style -> gp_xr { x_bn.text = x_text; x_bn.style = x_style } }", @wu_bn, "Say '#{@wv_msg}'", "-fx-font-family : '#{nu_fnt_nm}'; -fx-font-size : 27px;"
    jy_gc "{ x_lb, x_text -> gp_xr { x_lb .setText x_text } }", @wu_lb, "#{nu_nt} Font name : #{nu_fnt_nm}"
    @wv_fnt_idx += 1
  end
  def wn_on_quit x_ev
