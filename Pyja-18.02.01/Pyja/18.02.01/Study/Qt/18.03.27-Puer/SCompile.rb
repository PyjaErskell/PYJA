...
module DBody
  def self.dp_it
    ...
    GC_LOG .info "C++ source file => #{@__dau_src_fn}"
    raise RuntimeError, 'C++ source file not exist !!!' unless gf_xi @__dau_src_fn
    __dap_compile
  ensure
    gp_rp @__dau_tmp_pn
  end
private
  def self.__dap_compile
    ...
    if CgJnaPlatform .is_windows
      ...
      bu2_x_fn = gf_pj @__dau_tmp_pn, 'release', "#{pu_src_jn}.exe"
      ...
      bu2_x_fn = gf_pj @__dau_tmp_pn, "#{pu_src_jn}.app", 'Contents', 'MacOS', pu_src_jn
      GC_LOG .info "Copying generated executable #{bu2_x_fn} to #{ gf_to_phs pu_x_pn } ..."
      gp_cp bu2_x_fn, pu_x_pn
    end
  end
