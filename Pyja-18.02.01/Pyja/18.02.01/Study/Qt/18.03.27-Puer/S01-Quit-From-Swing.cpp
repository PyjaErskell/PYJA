...
namespace Global {
  ...
  ap_br (
    jy_ge ( CgQS ( R"(
      %1 = { final int x_ct = DgQt.DC_CT_AC, final long x_qo, final String x_mn, final Object... x_args ->
...
class WMain : public QObject {
  Q_OBJECT
public :
  jobject wl_frame = nullptr;
  explicit WMain ( QObject * xtl_parent = Q_NULLPTR ) : QObject (xtl_parent) { wn_init (); }
  void wn_init () { jy_ge ( CgQS ( "Thread .start { gp_sr { gp_qn %1, 'wn_sw_start', new javax.swing.JFrame () } } " ) .arg (ZC_QO) ); }
  Q_INVOKABLE void wn_sw_start ( at_cr (jobject) xr_frame ) {
    static at_ca nsf_new_frame_el = jy_ge ( R"( { x_qo ->
      return new java.awt.event.WindowAdapter () {
        void windowClosing ( final java.awt.event.WindowEvent x_ev ) {
          ...
          gp_qn x_qo, 'wn_sw_quit'
        }
      }
    } )" );
    wl_frame = af_tap ( xr_frame, {
      jy_gm ( it, "setTitle", { jf_ns (GC_APP_NM) } );
      jy_gm ( it, "setSize", { jf_ni (300), jf_ni (200) } );
      jy_gm ( it, "setLocationRelativeTo", {JC_NULL} );
      jy_gm ( it, "addWindowListener", { jy_gc ( nsf_new_frame_el, { jf_nl (ZC_QO) } ) } );
      jy_gm ( it, "setVisible", {JC_TRUE} );
    } );    
  }
  Q_INVOKABLE void wn_sw_quit () {
    JC_LOG::info ( "(Qt) About to quit" );
    AC_QAPP .quit ();
  }
};

#include "S01-Quit-From-Swing.moc"
