    ...
    __sap_fx_start = { final x_app, final x_stage ->
      ...
      final pu_bn = new Button () .with {
        ...
        onAction = { bu3_ev ->
          pv_cnt ++
          GC_LOG .info text
          gp_qn pu_qo, 'wn_bn', pv_cnt, { x4_text -> gp_xr { text = x4_text } }
        }
        it
      }
      ...
      pu_stage .with {
        ...
        onCloseRequest = { gp_qn GC_QAPP, 'quit' }
        show ()
      }
    }
    Thread .start { CgFxApp .csn_launch (__sap_fx_start) }
  )" ) .arg (ZC_QO) ); }
  zt_call wn_bn ( zt_o(x_cnt), zt_o(xp_it) ) {
    jy_gc ( xp_it, { jf_ns ( CgQS ( "(%1) Hello World from Qt" ) .arg ( jf_i (x_cnt) + 1 ) ) } );
  }
};

#include "S03-JavaFX-Hello.moc"
