...
jy_ge ('''
  ...
  Object.metaClass.{0} = {{ final long x_yi, final String x_nethod_nm, final Object... x_args ->
    synchronized (__gau_lock_4_{0}) {{ __gau_queue_4_{0} << [ x_yi, x_nethod_nm, x_args ] }}
  }}
  {0}_from_queue = {{ ->
    synchronized (__gau_lock_4_{0}) {{
      __gau_queue_4_{0} .each {{
        def ( bu2_yi, bu2_nethod_nm, bu2_args ) = it
        GC_JEP .invoke ( '{0}', bu2_yi, bu2_nethod_nm, *bu2_args )
      }}
      __gau_queue_4_{0} .clear ()
    }}
  }} 
''' .format ('gp_yn') )
class __CjYnProcessor (QObject) :
  def __init__ (self) :
    super () .__init__ ()
    self.startTimer (1)
  def timerEvent ( self, x_ev ) :
    jy_gf ( 'gp_yn_from_queue' )
JC_YN_PROCESSOR = __CjYnProcessor ()

jy_ge ( 'gp_sr = { final Closure xp_it -> javax.swing.SwingUtilities .invokeLater { xp_it () } }' )
jy_ge ('''
  Object.metaClass.gp_xr = { final Closure xp_it -> javafx.application.Platform .runLater { xp_it () } }
...
class WAtFxMain (CjAt) :
  ...
  def __init__ ( self, x_atr_qt_main ) :
    super () .__init__ ()
  ...
  def wn_init (self) :
    ...
    jy_ge ( f'''
      ...
      __sap_fx_start = {{ final x_app, final x_stage ->
        ...
        final pu_root = new VBox ()
        final pu_bn = new Button ()
        final pu_lb = new Label ()
        final pu_fnt_families = Font .families
        def pv_fnt_idx = pu_fnt_families .size () - 1
        def pv_msg = ''
        final pp2_move_center = {{ -> gp_xr {{
          final pu2_vb = Screen .primary .visualBounds
          x_stage .with {{
            x = pu2_vb.width / 2 - width / 2
            y = pu2_vb.height / 2 - height
          }}
        }} }}
        final pp2_change_font = {{ x2_tell = true -> gp_xr {{
          if ( pv_fnt_idx >= pu_fnt_families .size () ) {{ pv_fnt_idx = 0 }}
          final pu2_fnt_nm = pu_fnt_families .get pv_fnt_idx
          if ( pv_msg != '' ) {{ GC_LOG .info ( "JavaFx -> ${{pv_msg}}" ) }}
          final pu2_nt = "(${{pv_fnt_idx+1}}/${{ pu_fnt_families .size () }})"
          pv_msg = "0^0 ${{pu2_nt}} (${{pu2_fnt_nm}})"
          pu_bn .text = "Say '${{pv_msg}}'"
          pu_bn .style = "-fx-font-family : '${{pu2_fnt_nm}}'; -fx-font-size : 17px;"
          pu_lb .text = "${{pu2_nt}} Font name : ${{pu2_fnt_nm}}"
          pp2_move_center ()
          pv_fnt_idx += 1
          gp_yn pu_yi, 'wn_tell', x2_tell
        }} }}
        gp_yn pu_yi, 'wn_set_gc_change_font', pp2_change_font
        pu_bn .onAction = {{ pp2_change_font () }}
        pp2_change_font (false)
        pu_root .with {{
          alignment = Pos.CENTER
          spacing = 10
          children .addAll pu_bn, pu_lb
        }}
        final pu_stage = x_stage .with {{
          title = "${{GC_APP_NM}} - JavaFx"
          scene = gf_new_scene {{ new Scene ( pu_root, 650, 250 ) }}
          onShown = {{
            pp2_move_center ()
            gp_yn pu_yi, 'wn_shown'
          }}
          onCloseRequest = {{ gp_yn pu_yi, 'wn_quit' }}
          show ()
          it
        }}
      }}
    ...
    ''' )
  def wn_set_gc_change_font ( self, x_gc_change_font ) : self.wu_gc_change_font = x_gc_change_font
  def wn_change_font ( self, x_tell = True ) : jy_gc ( self.wu_gc_change_font, x_tell )
  def wn_tell ( self, x_tell = True ) :
  ...
  def receive ( self, x_letter, x_atr_sender ) :
    ...
    if ( nu_lcn == 'LNextFont' ) : self. wn_change_font (False)
  def wn_quit (self) : GC_QAPP .quit ()
...
class WAtQtMain (CjAt) :
  ...
  def wn_init (self) :
    ...
    self .wn_move_center ()
  def preStart (self) :
    self.wu_atr_fx_main = jf_mk_atr ( WAtFxMain ( self .getSelf () ), ':c' )
  def receive ( self, x_letter, x_atr_sender ) :
    nu_l = x_letter
    nu_lcn = nu_l.java_name
    JC_LOG .debug ( f'{self.__class__.__name__} : Received {nu_lcn}' )
    if ( nu_lcn == 'LAtrFxMainCreated' ) :
      self.wu_wgt .setEnabled (True)
      self.startTimer (90)
    elif ( nu_lcn == 'LNextFont' ) : self.wn_change_font (False)
  def timerEvent ( self, x_ev ) :
    self.wu_pb .animateClick ()
  def wn_tell ( self, x_tell = True ) :
