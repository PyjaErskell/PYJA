...
class WAtFxMain (CjAt) :
  ...
  def wn_display ( self, x_msg ) :
    JC_LOG .info (x_msg)
    self.wu_lv .getItems () .add (x_msg)
  def preStart (self) :
  ...
  def wn_fx_start ( self, x_app, x_stage ) :
    ...
    self.wu_stg .setWidth (555)
    self.wu_stg .setHeight (257)
    self.wu_stg .setScene (self.wu_scene)
    self.wu_stg .show ()
    jp_xr ( self.wn_do )
  def wn_do (self) :
    sqlhub.processConnection = connectionForURI ('sqlite:/:memory:')
    class TMessage (SQLObject) :
      co_sender = StringCol ()
      co_content = StringCol ()
    JC_LOG .info ( 'Creating database table ...' )
    TMessage .createTable (ifNotExists=True)
    [
      TMessage ( co_sender = "Dave", co_content = "Hello, HAL. Do you read me, HAL?" ),
      TMessage ( co_sender = "HAL",  co_content = "Affirmative, Dave. I read you." ),
      TMessage ( co_sender = "Dave", co_content = "Open the pod bay doors, HAL." ),
      TMessage ( co_sender = "HAL",  co_content = "I'm sorry, Dave. I'm afraid I can't do that." ),
    ]
    gp_log_array ( self.wn_display, 'Selecting all messages', TMessage .select () )
    gp_log_array ( self.wn_display, 'Selecting only messages from HAL', TMessage .select ( TMessage.q.co_sender == 'HAL' ) )
