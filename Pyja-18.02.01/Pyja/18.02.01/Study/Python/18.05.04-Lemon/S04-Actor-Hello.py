...
class CAtPrinter (CjAt) :
  LGreeting = jy_ge ( "class {0} {{ final String lu_it; {0} ( final String x_it ) {{ lu_it = x_it }} }}" .format ('LGreeting') )
  ...
  def receive ( self, x_letter, x_atr_sender ) :
    ...
    if ( nu_lcn == 'LGreeting' ) :
      bu2_it = jf_ga ( nu_l, 'lu_it' )
      self .tell ( self.cu_atr_main, WAtFxMain .LDisplay (bu2_it) )
      JC_LOG .info (bu2_it)

class CAtGreeter (CjAt) :
  LWhoToGreet = jy_ge ( "class {0} {{ final String lu_who; {0} ( final String x_who ) {{ lu_who = x_who }} }}" .format ('LWhoToGreet') )
  LGreet = jy_ge ( "class {0} {{}}" .format ('LGreet') )
  ...
  def receive ( self, x_letter, x_atr_sender ) :
    ...
    nu_lcn = nu_l.java_name
    JC_LOG .debug ( f'{self.__class__.__name__} : Received {nu_lcn}' )
    if ( nu_lcn == 'LWhoToGreet' ) : self.cv_greeting = f"{self.cu_msg} { jf_ga ( nu_l, 'lu_who' ) }"
    elif ( nu_lcn == 'LGreet' ) : self .tell ( self.cu_atr_printer, CAtPrinter .LGreeting (self.cv_greeting) )

class WAtFxMain (CjAt) :
  LDisplay = jy_ge ( "class {0} {{ final String lu_msg; {0} ( final String x_msg ) {{ lu_msg = x_msg }} }}" .format ('LDisplay') )
  def __init__ (self) : super () .__init__ ()
  def receive ( self, x_letter, x_atr_sender ) :
    nu_l = x_letter
    nu_lcn = nu_l.java_name
    JC_LOG .debug ( f'{self.__class__.__name__} : Received {nu_lcn}' )
    if ( nu_lcn == 'LDisplay' ) : jp_xr ( self.wn_display, jf_ga ( nu_l, 'lu_msg' ) )
  ...
  def wn_fx_start ( self, x_app, x_stage ) :
    self.wu_stg = x_stage
    jp_fx_set_on ( self.wu_stg.setOnShown, self.wn_shown )
    ...
    self.wu_lv = jf_jcls ('javafx.scene.control.ListView') ()
  ...
  def wn_do (self) :
    ...
    self.wu_atr_printer  = jf_mk_atr ( CAtPrinter ( self .getSelf () ), ':c' )
    self.wu_atr_hello    = jf_mk_atr ( CAtGreeter ( '-----HELLOHELLOHELLO-----', self.wu_atr_printer ), ':a' )
    self.wu_atr_howdy    = jf_mk_atr ( CAtGreeter ( 'HOWDY', self.wu_atr_printer ), ':a' )
    self.wu_atr_good_day = jf_mk_atr ( CAtGreeter ( 'HAVE*A*GOOD*DAY', self.wu_atr_printer ), ':a' )
  ...
  def wn_shown ( self, x_ev ) :
    JC_LOG .info ( f'{self.__class__.__name__} => OnShown' )
  def wn_quit ( self, x_ev ) : GC_QAPP .quit ()

class DBody :
  @classmethod
  def dp_it (cls) :
    pu_atr_main = jf_mk_atr ( WAtFxMain (), ':c' )
    GC_QAPP .exec_ ()
