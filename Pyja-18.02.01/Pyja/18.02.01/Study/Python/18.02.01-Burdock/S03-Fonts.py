...
class WAtFxMain (CgAt) :
  LNextFont = namedtuple ( 'LNextFont', [] )
  ...
  def wn_init (self) :
    ...
    self.wu_root = gf_jcls ('javafx.scene.layout.VBox') ()
    self.wu_scene = gf_jcls ('javafx.scene.Scene') (self.wu_root)
    self.wu_bn = gf_jcls ('javafx.scene.control.Button') ()
    self.wu_lb = gf_jcls ('javafx.scene.control.Label') ()
    self.wu_fnt_families = gf_jcls ('javafx.scene.text.Font') .getFamilies ()
    GC_LOG .info ( f'{self.__class__.__name__} : Total fonts => {len(self.wu_fnt_families)}' )
    self.wv_fnt_idx = len(self.wu_fnt_families) - 1
    self.wv_msg = ''
    gp_fx_set_on ( self.wu_bn.setOnAction, self.won_change_font )
    self.wn_change_font ( None, False )
    self.wu_root .setAlignment ( gf_jcls ('javafx.geometry.Pos') . CENTER )
  ...
  def receive ( self, x_yi_letter, x_sender_atr ) :
    ...
    if isinstance ( nu_letter, self.LNextFont ) : gp_xr ( self.wn_change_font, None, False )
  def wn_move_center (self) :
    nu_vb = gf_jcls ('javafx.stage.Screen') .getPrimary () .getVisualBounds ()
  ...
  def won_change_font ( self, x_ev ) : self .wn_change_font (x_ev)
  def wn_change_font ( self, x_ev, x_tell = True ) :
    ...
    if self.wv_msg != '' : GC_LOG .info ( f'JavaFx -> {self.wv_msg}' )
...
class WAtQtMain (CgAt) :
  ...
  def wn_init (self) :
    ...
    self.wu_wgt .raise_ ()
    self .wn_move_center ()
  def preStart (self) : gp_fx_start (self.wn_fx_start)
  def receive ( self, x_yi_letter, x_sender_atr ) :
    nu_letter = gf_yo ( x_yi_letter, True )
    GC_LOG .debug ( f'{self.__class__.__name__} : Received {nu_letter}' )
    if isinstance ( nu_letter, self.LAtrFxMainCreated ) : gp_qr ( self.wu_wgt .setEnabled, True )
    elif isinstance ( nu_letter, self.LNextFont ) : gp_qr ( self.wn_change_font, False )
  def wn_move_center (self) :
  ...
  def wn_fx_start ( self, x_app, x_stage ) :
    self.wu_atr_fx_main = gf_mk_atr ( WAtFxMain ( self .getSelf (), x_stage ), ':c' )
