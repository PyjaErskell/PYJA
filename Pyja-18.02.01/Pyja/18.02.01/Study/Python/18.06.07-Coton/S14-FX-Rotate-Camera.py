...
KeyFrame          = jf_jcls ('javafx.animation.KeyFrame')
KeyValue          = jf_jcls ('javafx.animation.KeyValue')
Timeline          = jf_jcls ('javafx.animation.Timeline')
Camera            = jf_jcls ('javafx.scene.Camera')
Group             = jf_jcls ('javafx.scene.Group')
Color             = jf_jcls ('javafx.scene.paint.Color')
PhongMaterial     = jf_jcls ('javafx.scene.paint.PhongMaterial')
PerspectiveCamera = jf_jcls ('javafx.scene.PerspectiveCamera')
Scene             = jf_jcls ('javafx.scene.Scene')
SceneAntialiasing = jf_jcls ('javafx.scene.SceneAntialiasing')
Box               = jf_jcls ('javafx.scene.shape.Box')
Sphere            = jf_jcls ('javafx.scene.shape.Sphere')
SubScene          = jf_jcls ('javafx.scene.SubScene')
Rotate            = jf_jcls ('javafx.scene.transform.Rotate')
Translate         = jf_jcls ('javafx.scene.transform.Translate')
Duration          = jf_jcls ('javafx.util.Duration')
...
class WAtFxMain ( CjAt, TjXu, TgWai ) :
  ...
  def wn_init (self) :
    ...
    def nf2_spr (it) :
      it .setMaterial ( PhongMaterial (Color.FORESTGREEN) )
      it .setTranslateZ (7)
      it .setTranslateX (2)
      return it
    self.wu_spr = nf2_spr ( Sphere (2.5) )
    def nf2_box (it) :
      it .setMaterial ( PhongMaterial (Color.RED) )
      return it
    self.wu_box = nf2_box ( Box ( 5, 5, 5 ) )
    self.wu_tra_pivot = Translate ()
    self.wu_rot_y = Rotate ( 0, Rotate.Y_AXIS )
    def nf2_cam (it) :
      it .getTransforms () .addAll ( [ self.wu_tra_pivot, self.wu_rot_y, Rotate ( -20, Rotate.X_AXIS ), Translate ( 0, 0, -50 ) ] )
      return it
    self.wu_cam = nf2_cam ( PerspectiveCamera (True) )
    def nf2_tml (it) :
      it .setCycleCount (Timeline.INDEFINITE)
      it .play ()
      return it
    self.wu_tml = nf2_tml ( Timeline ( jf_ja ( KeyFrame,
      KeyFrame ( Duration .seconds (0),  jf_ja ( KeyValue, KeyValue ( self.wu_rot_y .angleProperty (), 0 ) ) ),
      KeyFrame ( Duration .seconds (15), jf_ja ( KeyValue, KeyValue ( self.wu_rot_y .angleProperty (), 360 ) ) )
    ) ) )
    def nf2_root (it) :
      it .getChildren () .addAll ( [ self.wu_cam, self.wu_box, self.wu_spr ] )
      for bu_node in filter (
        lambda bx_node : not jf_is_instance ( Camera, bx_node ),
        it .getChildren () .stream () .toArray ()
      ) : self .tn_set_fx_eh ( bu_node, 'onMouseClicked', 'wn_on_mouse_clicked' )
      return it
    self.wu_root = nf2_root ( Group () )
    def nf2_sub_scene (it) :
      it .setFill (Color.ALICEBLUE)
      it .setCamera (self.wu_cam)
      return it
    self.wu_sub_scene = nf2_sub_scene ( SubScene ( self.wu_root, 300, 300, True, SceneAntialiasing.BALANCED ) )
    def nf2_grp (it) :
      it .getChildren () .add (self.wu_sub_scene)
      return it
    self.wu_grp = nf2_grp ( Group () )
    def nf2_stage (it) :
      it .setTitle (GC_APP_NM)
      it .setResizable (False)
      it .setScene ( Scene (self.wu_grp) )
  ...
  def wn_on_mouse_clicked ( self, x_ev ) :
    nu_pv = self.wu_tra_pivot
    nu_src = x_ev .getSource ()
    nu_pv .setX ( nu_src .getTranslateX () )
    nu_pv .setY ( nu_src .getTranslateY () )
    nu_pv .setZ ( nu_src .getTranslateZ () )
