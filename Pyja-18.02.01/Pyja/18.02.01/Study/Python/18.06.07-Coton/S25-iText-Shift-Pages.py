...
class WMain ( QMainWindow, TgWai ) :
  ...
  def wn_init (self) :
    ...
    def nf2_ui_add_config (x_row) :
      ...
      def ff3_new_dsb (x3_px) :
        ...
        return fu3_dsb
      def ff3_new_cb_duplex () :
        fu3_cb = QCheckBox ('Duplex')
        return fu3_cb
      def ff3_new_cb_left_binding () :
        fu3_cb = QCheckBox ('Left binding')
        fu3_cb .setVisible (False)
        fu3_cb .setChecked (True)
        return fu3_cb
      self.wu_dsb_hor = ff3_new_dsb ('⇨')
      self.wu_dsb_ver = ff3_new_dsb ('⇧')
      self.wu_cb_duplex = ff3_new_cb_duplex ()
      self.wu_cb_left_binding = ff3_new_cb_left_binding ()
      self.wu_hbl_cfg .addWidget (self.wu_dsb_hor)
      self.wu_hbl_cfg .addItem ( QSpacerItem ( 10, 1, QSizePolicy.Fixed, QSizePolicy.Minimum ) )
      self.wu_hbl_cfg .addWidget (self.wu_dsb_ver)
      self.wu_hbl_cfg .addItem ( QSpacerItem ( 10, 1, QSizePolicy.Fixed, QSizePolicy.Minimum ) )
      self.wu_hbl_cfg .addWidget (self.wu_cb_duplex)
      self.wu_hbl_cfg .addItem ( QSpacerItem ( 10, 1, QSizePolicy.Fixed, QSizePolicy.Minimum ) )
      self.wu_hbl_cfg .addWidget (self.wu_cb_left_binding)
    ...
    self.wu_dsb_hor.valueChanged .connect (self.wn_config_changed)
    self.wu_dsb_ver.valueChanged .connect (self.wn_config_changed)
    self.wu_cb_duplex.stateChanged .connect ( lambda bx2_stage : self .wn_config_changed () )
    self.wu_cb_left_binding.stateChanged .connect ( lambda bx2_stage : self .wn_config_changed () )
  ...
  def wn_config_changed (self) :
    ...
    nu_duplex = self.wu_cb_duplex .isChecked ()
    nu_left_binding = self.wu_cb_left_binding .isChecked ()
    self.wu_cb_left_binding .setVisible (nu_duplex)
    nu_shift_hor = self.wu_dsb_hor .value ()
    nu_shift_ver = self.wu_dsb_ver .value ()
    if self.wv_i_nop < 1 or ( nu_shift_hor == 0 and nu_shift_ver == 0 ) :
      self.wu_le_o_bn .setText ('')
      self.wu_pb_do .setEnabled (False)
      return
    nu_dos = 'Simplex' if not nu_duplex else 'DuplexLeftBinding' if nu_left_binding else 'DuplexTopBinding'
    self.wu_le_o_bn .setText ( f'{ gf_jn ( self.wu_le_i_bn .text () ) }-(ShiftPages)-(H{nu_shift_hor:.2f}V{nu_shift_ver:.2f}mm)-({nu_dos})-{nu_o_nop:d}-page(s).pdf' )
  ...
  def wn_pb_do_clicked (self) :
    ...
    def nf2_mb ( x2_org_mb, x2_shift_hor, x2_shift_ver ) :
      fu2_x = min ( x2_org_mb .getX (), x2_org_mb .getX () + x2_shift_hor )
      fu2_y = min ( x2_org_mb .getY (), x2_org_mb .getY () + x2_shift_ver )
      fu2_w = x2_org_mb .getWidth () + abs (x2_shift_hor)
      fu2_h = x2_org_mb .getHeight () + abs (x2_shift_ver)
      return CjRectangle ( fu2_x, fu2_y, fu2_w, fu2_h )
    def np2_do () :
      ...
      try :
        ...
        try :
          bu4_shift_hor = nf2_mm2pt ( self.wu_dsb_hor .value () )
          bu4_shift_ver = nf2_mm2pt ( self.wu_dsb_ver .value () )
          bu4_duplex = self.wu_cb_duplex .isChecked ()
          bu4_simplex = not bu4_duplex
          bu4_left_binding = self.wu_cb_left_binding .isChecked ()
          bu4_top_binding = not bu4_left_binding
          self .wn_log_info ( f'Horizontal shift => {bu4_shift_hor} point')
          self .wn_log_info ( f'Vertical shift => {bu4_shift_ver} point')
          ...
          for bu5_i_pg_no in bu4_page_no_list :
            bu5_even = True if bu5_i_pg_no % 2 == 0 else False
            bu5_x = bu4_shift_hor if bu4_simplex else -bu4_shift_hor if ( bu5_even and bu4_left_binding ) else bu4_shift_hor
            bu5_y = bu4_shift_ver if bu4_simplex else -bu4_shift_ver if ( bu5_even and bu4_top_binding ) else bu4_shift_ver
            ...
            bu5_o_pg = bu4_o_doc .addNewPage ( CjPageSize (bu5_i_pg_mb) )
            # bu5_o_pg .setMediaBox ( nf2_mb ( bu5_i_pg_mb, bu4_shift_hor, bu4_shift_ver ) )
            bu5_o_pg .setMediaBox ( nf2_mb ( bu5_i_pg_mb, bu5_x, bu5_y ) )
            bu5_o_pg .setBleedBox (bu5_i_pg_bb)
            bu5_o_pg .setTrimBox (bu5_i_pg_tb)
            bu5_o_pg .setCropBox (bu5_i_pg_cb)
            CjPdfCanvas (bu5_o_pg) .addXObject ( bu5_o_pg_cp, bu5_x, bu5_y )
            bv4_o_nop_so_far += 1
