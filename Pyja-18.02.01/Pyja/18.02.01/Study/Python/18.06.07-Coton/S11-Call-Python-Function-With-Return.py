...
CjFxWebView   = jf_jcls ('javafx.scene.web.WebView')

class WFxMain ( CgQo, TgWai ) :
  def __init__ (self) :
    super () .__init__ ()
    self .wn_init ()
  def wn_init (self) :
    ...
    self.wu_iv_qt_svg = jy_gc ('''{ x_yi, x_fn ->
      ...
      final fu_iv = new javafx.scene.image.ImageView (fu_img)
      fu_iv .with {
        preserveRatio = true
        smooth = true
        fitWidth = 300
    ...
    self.wu_wv_license .getEngine () .loadContent ('''
      <center>
        <div>Icons made by <a href="https://www.flaticon.com/authors/yannick" title="Yannick">Yannick</a> 
    ...
    self.wu_root .getChildren () .addAll ( [ self.wu_iv_qt_svg, self.wu_iv_fx_svg, self.wu_wv_license ] )
    jy_gc ('''{ x_yi, x_stage ->
      x_stage .with {
    ...
    self.wu_stage .show ()
    self .wn_move_center ()
  def wm_pixmap ( self, x_fn ) : return jf_pixmap (x_fn)
