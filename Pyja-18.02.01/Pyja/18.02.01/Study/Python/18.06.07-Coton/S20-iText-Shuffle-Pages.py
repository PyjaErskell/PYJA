...
class WMain ( QMainWindow, TgWai ) :
  ...
  def wn_set_output_fn (self) :
    ...
    self.wu_le_o_bn .setText ( f'{ gf_jn ( self.wu_le_i_bn .text () ) }-(ShufflePages)-{nu_o_nop:d}-page(s).pdf' )
    ...
    def np2_do () :
      ...
      try :
        ...
        try :
          ...
          bv4_o_nop_so_far = 0
          bv4_page_no_list = list ( range ( 1, bu3_i_nop+1 ) )
          random .shuffle (bv4_page_no_list)
          for bu5_i_pg_no in bv4_page_no_list :
            bu5_i_pg = bu4_i_doc .getPage (bu5_i_pg_no)
            bu5_i_pg_bb_sz = bu5_i_pg .getBleedBox ()
            bu5_i_pg_tb_sz = bu5_i_pg .getTrimBox ()
            bu5_i_pg .setCropBox (bu5_i_pg_bb_sz)
            bu5_o_pg_cp = bu5_i_pg .copyAsFormXObject (bu4_o_doc)
            bu5_o_pg = bu4_o_doc .addNewPage ( CjPageSize (bu5_i_pg_bb_sz) )
