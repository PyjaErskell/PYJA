...
class WMain ( QMainWindow, TgWai ) :
  ...
  def wn_set_output_fn (self) :
    ...
    self.wu_le_o_bn .setText ( f'{ gf_jn ( self.wu_le_i_bn .text () ) }-(VerticalMirrorPages)-{nu_o_nop:d}-page(s).pdf' )
  ...
  def wn_pb_do_clicked (self) :
    ...
    def nf2_ver_mirror_box ( x2_media_bx, x2_bx ) :
      return CjRectangle ( x2_bx .getX (), x2_media_bx .getTop () - ( x2_bx .getY () - x2_media_bx .getY () + x2_bx .getHeight () ), x2_bx .getWidth (), x2_bx .getHeight () )
    def np2_do () :
      ...
      try :
        ...
        try :
          ...
          for bu5_i_pg_no in bu4_page_no_list :
            ...
            bu5_o_pg .setBleedBox ( nf2_ver_mirror_box ( bu5_i_pg_mb_sz, bu5_i_pg_bb_sz ) )
            bu5_o_pg .setTrimBox ( nf2_ver_mirror_box ( bu5_i_pg_mb_sz, bu5_i_pg_tb_sz ) )
            bu5_o_pg .setCropBox ( nf2_ver_mirror_box ( bu5_i_pg_mb_sz, bu5_i_pg_cb_sz ) )
            CjPdfCanvas (bu5_o_pg) .addXObject ( bu5_o_pg_cp, 1, 0, 0, -1, 0, bu5_i_pg_mb_sz .getHeight () )
            bv4_o_nop_so_far += 1
