...
Button               = jf_jcls ('javafx.scene.control.Button')
Font                 = jf_jcls ('javafx.scene.text.Font')
FXCollections        = jf_jcls ('javafx.collections.FXCollections')
HBox                 = jf_jcls ('javafx.scene.layout.HBox')
Insets               = jf_jcls ('javafx.geometry.Insets')
Label                = jf_jcls ('javafx.scene.control.Label')
Pos                  = jf_jcls ('javafx.geometry.Pos')
Priority             = jf_jcls ('javafx.scene.layout.Priority')
PropertyValueFactory = jf_jcls ('javafx.scene.control.cell.PropertyValueFactory')
TableColumn          = jf_jcls ('javafx.scene.control.TableColumn')
TableView            = jf_jcls ('javafx.scene.control.TableView')
TextField            = jf_jcls ('javafx.scene.control.TextField')
TextFieldTableCell   = jf_jcls ('javafx.scene.control.cell.TextFieldTableCell')
VBox                 = jf_jcls ('javafx.scene.layout.VBox')

class HPerson :
  hcu_items = FXCollections.observableArrayList ()
  HIt = jy_ge ( """
    import javafx.beans.property.SimpleStringProperty
    class HIt {
      final private SimpleStringProperty __hau_first_nm
      final private SimpleStringProperty __hau_last_nm
      final private SimpleStringProperty __hau_email
      HIt ( final String x_first_nm, final String x_last_nm, final String x_email ) {
        __hau_first_nm = new SimpleStringProperty (x_first_nm)
        __hau_last_nm = new SimpleStringProperty (x_last_nm)
        __hau_email = new SimpleStringProperty (x_email)
      }
      def getHv_first_nm () { return __hau_first_nm .get () }
      void setHv_first_nm ( final String x_it ) { __hau_first_nm .set x_it }
      def  getHv_last_nm () { return __hau_last_nm .get () }
      void setHv_last_nm ( final String x_it ) { __hau_last_nm .set x_it }
      def  getHv_email () { return __hau_email .get () }
      void setHv_email ( final String x_it ) { __hau_email .set x_it }
      def hm_2s () {
        [ __hau_first_nm, __hau_last_nm, __hau_email ] .collect { it .get () }
      }
    }
  """)
...
] : HPerson.hcu_items .add ( HPerson.HIt (*bu2_it) )

class WAtFxMain ( CjAt, TjXu, TgWai ) :
  def __init__ (self) : super () .__init__ ()
  def create (self) : self .wn_init ()
  def wn_init (self) :
    JC_LOG .info ( self.tm_wai ('Initializing ...') )
    self.wu_root = jf_jcls ('javafx.scene.layout.StackPane') ()
    self.wu_scene = jf_jcls ('javafx.scene.Scene') ( self.wu_root, 460, 570 )
    def nf2_vb (x2_vb) :
      def pf3_lb (it) :
        it .setText ( 'Address Book' )
        it .setFont ( Font ( 'Arial', 13.5 ) )
        return it
      self.wu_lb = pf3_lb ( Label () )
      def pf3_tv (x3_tv) :
        def ff4_tc ( x4_txt, x4_mw, x4_a ) :
          fu4_tc = TableColumn ()
          fu4_tc .setText (x4_txt)
          fu4_tc .setMinWidth (x4_mw)
          fu4_tc .setCellFactory ( TextFieldTableCell .forTableColumn () )
          fu4_tc .setCellValueFactory ( PropertyValueFactory (x4_a) )
          self .tn_set_fx_eh ( fu4_tc, 'onEditCommit', 'wn_on_edit_commit' )
          return fu4_tc
        self.wu_tc_first_nm, self.wu_tc_last_nm, self.wu_tc_email = map (
          lambda it : ff4_tc (*it), [
          [ 'First Name', 100, 'hv_first_nm' ],
          [ 'Last Name', 100, 'hv_last_nm' ],
          [ 'Email', 200, 'hv_email' ],
        ] )
        x3_tv .setEditable (True)
        x3_tv .setColumnResizePolicy (TableView.CONSTRAINED_RESIZE_POLICY)
        VBox .setVgrow ( x3_tv, Priority.ALWAYS )
        x3_tv .getColumns () .addAll ( [ self.wu_tc_first_nm, self.wu_tc_last_nm, self.wu_tc_email ] )
        x3_tv .setItems (HPerson.hcu_items)
        return x3_tv
      self.wu_tv = pf3_tv ( TableView () )
      def pf3_hb (x3_hb) :
        def ff4_tf ( x4_txt, x4_tc ) :
          fu4_tf = TextField ()
          fu4_tf .setPromptText (x4_txt)
          fu4_tf .setMaxWidth ( x4_tc .getPrefWidth () )
          return fu4_tf
        self.wu_tf_first_nm, self.wu_tf_last_nm, self.wu_tf_email = map (
          lambda it : ff4_tf (*it), [
          [ 'First Name', self.wu_tc_first_nm ],
          [ 'Last Name', self.wu_tc_last_nm ],
          [ 'Email', self.wu_tc_email ],
        ] )
        def ff4_bn_add (it) :
          it .setText ('Add')
          self .tn_set_fx_eh ( it, 'onAction', 'wn_on_add' )
          return it
        self.wu_bn_add = ff4_bn_add ( Button () )
        x3_hb .setSpacing (3)
        x3_hb .setAlignment (Pos.CENTER)
        x3_hb .getChildren () .addAll ( [ self.wu_tf_first_nm, self.wu_tf_last_nm, self.wu_tf_email, self.wu_bn_add ] )
        return x3_hb
      self.wu_hb = pf3_hb ( HBox () )
      x2_vb .setSpacing (5)
      x2_vb .setPadding ( Insets (10) )
      x2_vb .setAlignment (Pos.CENTER)
      x2_vb .getChildren () .addAll ( [ self.wu_lb, self.wu_tv, self.wu_hb ] )
      return x2_vb
    self.wu_vb = nf2_vb ( VBox () )
    self.wu_root .getChildren () .addAll ([self.wu_vb])
    def nf2_stage (it) :
      it .setTitle (GC_APP_NM)
      it .setScene (self.wu_scene)
  ...
  def wn_on_edit_commit ( self, x_ev ) :
    nu_o = x_ev .getTableView () .getItems () .get ( x_ev .getTablePosition () .getRow () )
    getattr ( nu_o, f'set{ x_ev .getSource () .getCellValueFactory () .getProperty () .capitalize () }' ) ( x_ev .getNewValue () )
    JC_LOG .info ( f'onEditCommit => { nu_o .hm_2s () }' )
  def wn_on_add ( self, x_ev ) :
    nu_tfs = [ self.wu_tf_first_nm, self.wu_tf_last_nm, self.wu_tf_email ]
    HPerson.hcu_items .add ( HPerson.HIt ( * map ( lambda it : it .getText (), nu_tfs ) ) )
    for bu2_tf in nu_tfs : bu2_tf .clear ()
  def wn_on_shown ( self, x_ev ) : JC_LOG .info ( self.tm_wai ( 'Stage shown ...' ) )
