...
class WMain ( QMainWindow, TgWai ) :
  ...
  def wn_set_output_fn (self) :
    ...
    self.wu_le_o_bn .setText ( f'{ gf_jn ( self.wu_le_i_bn .text () ) }-(HorizontalMirrorPages)-{nu_o_nop:d}-page(s).pdf' )
  ...
  def wn_pb_do_clicked (self) :
    ...
    def nf2_hor_mirror_box ( x2_media_bx, x2_bx ) :
      return CjRectangle ( x2_media_bx .getRight () - ( x2_bx .getX () - x2_media_bx .getX () + x2_bx .getWidth () ), x2_bx .getY (), x2_bx .getWidth (), x2_bx .getHeight () )
    def np2_do () :
      ...
      try :
        ...
        try :
          ...
          for bu5_i_pg_no in bu4_page_no_list :
            ...
            bu5_o_pg = bu4_o_doc .addNewPage ( CjPageSize (bu5_i_pg_mb_sz) )
            bu5_o_pg .setMediaBox (bu5_i_pg_mb_sz)
            bu5_o_pg .setBleedBox ( nf2_hor_mirror_box ( bu5_i_pg_mb_sz, bu5_i_pg_bb_sz ) )
            bu5_o_pg .setTrimBox ( nf2_hor_mirror_box ( bu5_i_pg_mb_sz, bu5_i_pg_tb_sz ) )
            bu5_o_pg .setCropBox ( nf2_hor_mirror_box ( bu5_i_pg_mb_sz, bu5_i_pg_cb_sz ) )
            CjPdfCanvas (bu5_o_pg) .addXObject ( bu5_o_pg_cp, -1, 0, 0, 1, bu5_i_pg_mb_sz .getWidth (), 0 )
            bv4_o_nop_so_far += 1
