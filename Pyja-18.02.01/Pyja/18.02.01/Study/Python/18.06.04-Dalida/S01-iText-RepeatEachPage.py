...
def gp_request_exit ( x_ec, x_ex_list = None ) :
  pu_ex_list = ...
...
jy_ge ('''
  ...
  gy_cy = { final String x_fun_nm, final Object... x_args -> GC_JEP .invoke ( x_fun_nm, *x_args ) } // (c)all p(y)thon
  addShutdownHook { gy_cy 'gp_request_exit', GC_EC_SHUTDOWN, 'Shutdown occurred !!!'  }
''')
...
class WMain ( QMainWindow, GtWai ) :
  ...
  def wn_pb_do_clicked (self) :
    ...
    try : np2_do ()
      ...
      if nv_cancelled : self .wn_log_warning ( f'Cancelled by user !!!' )
  ...
  def wn_fini (self) : GC_LOG .info ( self.tm_wai ('Finalizing ...') )
