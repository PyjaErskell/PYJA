...
class CAtFibonacci < CgAt
  ...
  def receive x_letter
    case x_letter
    when Integer then sender .tell @cu_it [x_letter], get_self
    end
  end
end
...
class WAtMain < CgAt
  ...
  def wn_init
    ...
    @wu_atr_fibo = gf_mk_atr CAtFibonacci, [], :c
    @wu_timeout = CgTimeout.new CgDuration.create 2, 'seconds'
    @wu_lb_no = Label.new
    nu_initial_no = 21
    wn_display_no nu_initial_no
    @wu_sldr_4_no = Slider.new .tap { |it|
      it.min = 0
      it.max = 360
    ...
    @wu_lb_fibo = Label.new 'Fibonacci :'
    @wu_lb_fibo_result = Label.new
    @wu_root = VBox.new .tap { |it|
      it.alignment = Pos::CENTER
      it.padding = Insets.new 20
      it.spacing = 10
  ...
  def wn_display_no x_no
    @wu_lb_no.text = "Number => #{x_no}"
  end
  def wn_display_fibo
    nu_future = CgPatterns.ask @wu_atr_fibo, @wu_sldr_4_no.value .to_i, @wu_timeout
    nu_result = CgAwait.result nu_future, @wu_timeout.duration
    @wu_lb_fibo_result.text = CgString.format '%,d', nu_result
  end
end
