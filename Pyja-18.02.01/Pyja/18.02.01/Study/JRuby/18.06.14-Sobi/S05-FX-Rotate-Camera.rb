...
class WAtMain < CgAt
  ...
  def wn_init
    ...
    @wu_spr = Sphere.new(2.5) .tap { |it|
      it .material = PhongMaterial.new Color::FORESTGREEN
      it .translate_z = 7
      it .translate_x = 2
    }
    @wu_box = ( Box.new 5, 5, 5 ) .tap { |it|
      it .material = PhongMaterial.new Color::RED
    }
    @wu_tra_pivot = Translate.new
    @wu_rot_y = Rotate.new 0, Rotate::Y_AXIS
    @wu_cam = ( PerspectiveCamera.new true ) .tap { |it|
      it.transforms .add_all @wu_tra_pivot, @wu_rot_y, ( Rotate.new -20, Rotate::X_AXIS ), ( Translate.new 0, 0, -50 )
    }
    @wu_tml = Timeline.new(
      ( KeyFrame.new ( Duration::seconds  0 ), ( KeyValue.new @wu_rot_y.angle_property,   0 ) ),
      ( KeyFrame.new ( Duration::seconds 15 ), ( KeyValue.new @wu_rot_y.angle_property, 360 ) )
    ) .tap { |it|
      it.cycle_count = Timeline::INDEFINITE
      it .play
    }
    @wu_root = Group.new .tap { |it|
      it.children .add_all [ @wu_cam, @wu_box, @wu_spr ]
      it.children.stream .filter { |bx_node|
        ! bx_node .java_kind_of? Camera
      } .for_each { |bx_node|
        bx_node .set_on_mouse_clicked { |bx_ev|
          @wu_tra_pivot.x = bx_node.translate_x
          @wu_tra_pivot.y = bx_node.translate_y
          @wu_tra_pivot.z = bx_node.translate_z
        }
      }
    }
    @wu_sub_scene = ( SubScene.new @wu_root, 300, 300, true, SceneAntialiasing::BALANCED ) .tap { |it|
      it.fill = Color::ALICEBLUE
      it.camera = @wu_cam
    }
    @wu_grp = Group.new .tap { |it|
      it.children .add @wu_sub_scene
    }
    @wu_stage = GC_FX_STAGE .tap { |it|
      it.title = GC_APP_NM
      it.resizable = false
      it.scene = Scene.new @wu_grp
