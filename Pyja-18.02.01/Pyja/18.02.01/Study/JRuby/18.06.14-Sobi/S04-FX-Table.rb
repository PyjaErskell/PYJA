...
class HPerson
  ...
  @hsu_items = FXCollections::observable_array_list

  def initialize x_first_nm, x_last_nm, x_email
    @__hau_first_nm = SimpleStringProperty.new x_first_nm
    @__hau_last_nm = SimpleStringProperty.new x_last_nm
    @__hau_email = SimpleStringProperty.new x_email
  end

  def hm_2s
    [ :@__hau_first_nm, :@__hau_last_nm, :@__hau_email ] .map { |bx2_it| ( self.instance_variable_get bx2_it ) .get }
  end

  java_signature 'java.lang.String getHv_first_nm ()'
  def getHv_first_nm; @__hau_first_nm .get; end
  java_signature 'void setHv_first_nm (java.lang.String)'
  def setHv_first_nm x_it; @__hau_first_nm .set x_it; end

  java_signature 'java.lang.String getHv_last_nm ()'
  def getHv_last_nm; @__hau_last_nm .get; end
  java_signature 'void setHv_last_nm (java.lang.String)'
  def setHv_last_nm x_it; @__hau_last_nm .set x_it; end

  java_signature 'java.lang.String getHv_email ()'
  def getHv_email; @__hau_email .get; end
  java_signature 'void setHv_email (java.lang.String)'
  def setHv_email x_it; @__hau_email .set x_it; end
...
] .each { |bx2_it| HPerson.hsu_items .add HPerson.new *bx2_it }
...
class WAtMain < CgAt
  ...
  def wn_init
    ...
    @wu_root = javafx.scene.layout.StackPane.new
    @wu_scene = javafx.scene.Scene.new @wu_root, 460, 570
    @wu_vb = VBox.new .tap { |bx2_vb|
      @wu_lb = Label.new .tap { |it|
        it.text = 'Address Book'
        it.font = Font.new 'Arial', 13.5
      }
      @wu_tv = TableView.new .tap { |bx3_tv|
        @wu_tc_first_nm = TableColumn.new .tap { |it|
          it.text = 'First Name'
          it.min_width = 100
          it.cell_factory = TextFieldTableCell::for_table_column
          it.cell_value_factory = PropertyValueFactory.new 'hv_first_nm'
        }
        @wu_tc_last_nm = TableColumn.new .tap { |it|
          it.text = 'Last Name'
          it.min_width = 100
          it.cell_factory = TextFieldTableCell::for_table_column
          it.cell_value_factory = PropertyValueFactory.new 'hv_last_nm'
        }
        @wu_tc_email = TableColumn.new .tap { |it|
          it.text = 'EMail'
          it.min_width = 200
          it.cell_factory = TextFieldTableCell::for_table_column
          it.cell_value_factory = PropertyValueFactory.new 'hv_email'
        }
        [ @wu_tc_first_nm, @wu_tc_last_nm, @wu_tc_email ] .each { |bx4_tc|
          bx4_tc .set_on_edit_commit { |bx5_ev|
            bu5_o = bx5_ev.table_view.items .get bx5_ev.table_position.row
            bu5_o.send "set#{bx5_ev.source.cell_value_factory.property.capitalize}", bx5_ev.new_value
            GC_LOG .info "onEditCommit => #{bu5_o.hm_2s}"
          }
        }
        bx3_tv .tap { |it|
          it.editable = true
          it.column_resize_policy = TableView::CONSTRAINED_RESIZE_POLICY
          VBox::set_vgrow it, Priority::ALWAYS
          it.columns .add_all [ @wu_tc_first_nm, @wu_tc_last_nm, @wu_tc_email ]
          it.items = HPerson.hsu_items
        }
      }
      @wu_hb = HBox.new .tap { |bx3_hb|
        @wu_tf_first_nm = TextField.new .tap { |it|
          it.prompt_text = 'First Name'
          it.max_width = @wu_tc_first_nm.pref_width
        }
        @wu_tf_last_nm = TextField.new .tap { |it|
          it.prompt_text = 'Last Name'
          it.max_width = @wu_tc_last_nm.pref_width
        }
        @wu_tf_email = TextField.new .tap { |it|
          it.prompt_text = 'EMail'
          it.max_width = @wu_tc_email.pref_width
        }
        @wu_bn_add = Button.new .tap { |it|
          it.text = 'Add'
          it.set_on_action {
            bu5_tfs = [ @wu_tf_first_nm, @wu_tf_last_nm, @wu_tf_email ]
            HPerson.hsu_items .add HPerson.new *( bu5_tfs.map { |bx6_tf|  bx6_tf.text } )
            bu5_tfs .each { |bx6_tf| bx6_tf .clear }
          }
        }
        bx3_hb .tap { |it|
          it.spacing = 3
          it.alignment = Pos::CENTER
          it.children .add_all [  @wu_tf_first_nm, @wu_tf_last_nm, @wu_tf_email, @wu_bn_add ]
        }
      }
      bx2_vb .tap { |it|
        it.spacing = 5
        it.padding = Insets.new 10
        it.alignment = Pos::CENTER
        it.children .add_all [ @wu_lb, @wu_tv, @wu_hb ]
      }
    }
    @wu_root.children .add_all [@wu_vb]
    @wu_stage .tap { |it|
      it.title = GC_APP_NM
      it.scene = @wu_scene
