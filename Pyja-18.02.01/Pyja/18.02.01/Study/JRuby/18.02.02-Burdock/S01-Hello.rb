...
class CAtGreeter < CgAt
  self.const_set :LWhoToGreet, ( Struct.new :lu_who )
  def create x_msg, x_atr_printer
    @cu_msg = x_msg
    @cu_atr_printer = x_atr_printer
  end
  def receive x_letter
    case x_letter
    when self.class::LWhoToGreet then @cv_greeting = "#{@cu_msg} #{x_letter.lu_who} !!!"
    when :LGreet then tell @cu_atr_printer, ( CAtPrinter::LGreeting.new @cv_greeting )
    end
  end
end

class CAtPrinter < CgAt
  self.const_set :LGreeting, ( Struct.new :lu_it )
  def create x_atr_main
    @cu_atr_main = x_atr_main
  end
  def receive x_letter
    case x_letter
    when self.class::LGreeting
      tell @cu_atr_main, ( WAtMain::LDisplay.new x_letter.lu_it )
      GC_LOG .info "#{get_sender.path} => #{x_letter.lu_it}"
    end
  end
end

class WAtMain < CgAt
  self.const_set :LDisplay, ( Struct.new :lu_msg )
  def create x_stage
    @wu_stg = x_stage
    gp_xr { wn_init }
  end
  def receive x_letter
    case x_letter
    when self.class::LDisplay then gp_xr { @wu_lv .items .add x_letter.lu_msg }
    end
  end
  def wn_init
    @wu_root = javafx.scene.layout.StackPane .new
    @wu_scene = javafx.scene.Scene .new @wu_root
    @wu_lv = javafx.scene.control.ListView.new
    @wu_root .children .add_all @wu_lv
    @wu_stg .tap { |it|
  ...
  def wn_shown x_ev
    ...
    nv_sn = 0
    np2_greet = ->( x2_it, x2_who ) {
      nv_sn += 1
      tell x2_it, ( CAtGreeter::LWhoToGreet.new "<#{ '%03d' % nv_sn }> #{x2_who}" )
      tell x2_it, :LGreet
    }
    @wu_atr_printer = gf_mk_atr CAtPrinter, [get_self], :c
    @wu_atr_hello = gf_mk_atr CAtGreeter, [ '-----HELLOHELLOHELLO-----', @wu_atr_printer ], :a
    @wu_atr_howdy = gf_mk_atr CAtGreeter, [ 'HOWDY', @wu_atr_printer ], :a
    @wu_atr_good_day = gf_mk_atr CAtGreeter, [ 'HAVE*A*GOOD*DAY', @wu_atr_printer ], :a
    [ 'JRuby', 'QtJambi', 'Groovy' ] .each { |bx2_who| np2_greet .call @wu_atr_hello, bx2_who }
    [ 'JavaFx', 'Sequel', 'Scala', 'Akka', 'Rails', 'Slick', 'Shoes' ] .each { |bx2_who| np2_greet .call @wu_atr_howdy, bx2_who }
    [ 'Programming with Python, JRuby, Scala, ...',
      ...
      'Pyja - Python rubY Java Anything',
      'Pyja - Python rubY Java Anything',
    ] .each { |bx2_who| np2_greet .call @wu_atr_good_day, bx2_who }
  end
...
module DBody
  def self.dp_it
    ...
    gp_fx_start { | bx2_app, bx2_stage | gf_mk_atr WAtMain, [bx2_stage], :c }
  end
end
