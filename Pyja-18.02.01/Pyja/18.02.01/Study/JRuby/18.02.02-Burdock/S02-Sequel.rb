...
class WAtMain < CgAt
  ...
  def wn_display x_msg
    GC_LOG .info x_msg
    @wu_lv .items .add x_msg
  end
  def wn_shown x_ev
    ...
    nu_db = Sequel.connect "jdbc:h2:mem:#{GC_APP_NM};DB_CLOSE_DELAY=-1"
    GC_LOG .info 'Creating database table ...'
    unless nu_db .table_exists? :t_message
      nu_db .create_table :t_message do
        String :co_sender
        String :co_content
      end
    end
    GC_LOG .info 'Inserting test data ...'
    nu_messages = nu_db [:t_message]
    nu_fresh_test_data .each { |it| nu_messages .insert *it }
    gp_log_array ( self.method :wn_display ), 'Selecting all messages', nu_messages .all
    gp_log_array ( self.method :wn_display ), 'Selecting only messages from HAL', ( nu_messages .where co_sender: 'HAL' ) .all
  end
...
module DBody
  ...
  ] .each { |bx2_jar_fn| gp_add_jar bx2_jar_fn }
  def self.dp_it
