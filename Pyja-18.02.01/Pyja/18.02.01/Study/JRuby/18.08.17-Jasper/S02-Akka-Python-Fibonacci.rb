...
def gf_ak_fut_oc &x_block # ak -> (ak)ka, fut -> (fut)ure, oc -> (o)n(C)omplete
...
class WAtMain < CgAt
  def create x_atr_fibo
    @wu_atr_fibo = x_atr_fibo
    gp_xr { wn_init }
  end
  def wn_init
  ...
  def wn_display_fibo
    ...
    nu_future .on_complete gf_ak_fut_oc { | x2_throwable, x2_result |
      if x2_throwable.nil?
        gp_xr {
          @wu_lb_fibo_result.text = CgString.format '%,d', x2_result 
        }
      end
    }, context.dispatcher
  end
