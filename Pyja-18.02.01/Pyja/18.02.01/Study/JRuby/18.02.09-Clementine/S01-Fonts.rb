...
GC_GR .eval <<-__GASH_EOS.unindent
  ...
  def gf_jep () { return __gau_jep }
__GASH_EOS
GC_JEP = gy_gr_call 'gf_jep'
GC_JEP .set 'GC_GR', GC_GR
GC_JEP .set 'GC_AS', GC_AS
GC_JEP .set 'GC_LOG', GC_LOG
__gau_py_fn = gf_pj GC_MILO_PN, "#{GC_APP_NM}.py"
GC_JEP .set 'GC_SCRIPT_FN', __gau_py_fn
GC_JEP .run_script (__gau_py_fn)

GC_CMD = GC_JEP .get_value 'GC_CMD'
GC_PY_VER = GC_JEP .get_value 'platform .python_version ()'
def gf_os_available_memory; GC_JEP .get_value 'gf_os_available_memory ()'; end
def gf_qapp_exec; GC_JEP .get_value 'GC_QAPP .exec_ ()'; end
...
private
  def self.__dap_begin
    ...
    GC_LOG .debug "Python version => #{GC_PY_VER}"
    GC_LOG .debug "Total CPU => #{GC_TOTAL_CPU}"
    GC_LOG .debug "Total memory => #{ CgString .format '%,d', GC_TOTAL_MEMORY } bytes"
    GC_LOG .debug "Available memory => #{ CgString .format '%,d', gf_os_available_memory } bytes"
    ...
    gp_log_array  ( GC_LOG .method :debug ), 'Command', GC_CMD if GC_CMD.count > 0
...
class WAtFxMain < CgAt
  def create x_stage, x_atr_qt_main
    @wu_stg = x_stage
    @wu_atr_qt_main = x_atr_qt_main
    gp_xr { wn_init }
  end
...
module DBody
  def self.dp_it
    ...
    pu_atr_qt_main = GC_JEP .invoke 'gf_mk_atr', 'WAtQtMain', [] .to_java, ':c'
    gp_fx_start { | bx2_app, bx2_stage | gf_mk_atr WAtFxMain, [ bx2_stage, pu_atr_qt_main ], :c }
  end
end
