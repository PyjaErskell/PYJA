...
class WAtQtMain (CgAt) :
  ...
  def wm_init_info (self) :
    ...
    mu_lo = QGridLayout ()
    mu_lo .addWidget ( QLabel ('Application name'), 0, 0, Qt.AlignRight | Qt.AlignCenter )
    mu_lo .addWidget ( QLabel ('Application version'), 1, 0, Qt.AlignRight | Qt.AlignCenter )
    mu_lo .addWidget ( QLabel ('Application author'), 2, 0, Qt.AlignRight | Qt.AlignCenter )
    def mf2_le (x_str) :
      fu2_it = QLineEdit ()
      fu2_it .setText (x_str)
      fu2_it .setReadOnly (True)
      return fu2_it
    mu_lo .addWidget ( mf2_le (self.wu_app_nm), 0, 1, Qt.AlignLeft | Qt.AlignCenter )
    mu_lo .addWidget ( mf2_le (self.wu_app_vr), 1, 1, Qt.AlignLeft | Qt.AlignCenter )
    mu_lo .addWidget ( mf2_le (self.wu_app_au), 2, 1, Qt.AlignLeft | Qt.AlignCenter )
    for bu2_row in range (3) : mu_lo .addItem ( QSpacerItem ( 1, 1, QSizePolicy.Expanding, QSizePolicy.Minimum ), bu2_row, 2 )
    return mu_lo
  def wm_init_appdirs (self) :
    ...
    mu_app_dirs = gf_jcls ('net.harawata.appdirs.AppDirsFactory') .getInstance ()
    mp2_set_item ( 0, 'User data path',     gf_ap ( mu_app_dirs.getUserDataDir   ( self.wu_app_nm, self.wu_app_vr, self.wu_app_au ) ) )
    mp2_set_item ( 1, 'User config path',   gf_ap ( mu_app_dirs.getUserConfigDir ( self.wu_app_nm, self.wu_app_vr, self.wu_app_au ) ) )
    mp2_set_item ( 2, 'User cache path',    gf_ap ( mu_app_dirs.getUserCacheDir  ( self.wu_app_nm, self.wu_app_vr, self.wu_app_au ) ) )
    mp2_set_item ( 3, 'User log path',      gf_ap ( mu_app_dirs.getUserLogDir    ( self.wu_app_nm, self.wu_app_vr, self.wu_app_au ) ) )
    mp2_set_item ( 4, 'Site data path',     gf_ap ( mu_app_dirs.getSiteDataDir   ( self.wu_app_nm, self.wu_app_vr, self.wu_app_au ) ) )
    mp2_set_item ( 5, 'Site config path',   gf_ap ( mu_app_dirs.getSiteConfigDir ( self.wu_app_nm, self.wu_app_vr, self.wu_app_au ) ) )
    mu_it .resizeColumnsToContents ()
    return mu_it
  def wn_move_center (self) :
    nu_cp = QDesktopWidget () .availableGeometry () .center () # center point
    self.wu_wgt .move ( nu_cp.x() - self.wu_wgt.width()/2, nu_cp.y() - self.wu_wgt.height()/2 )
