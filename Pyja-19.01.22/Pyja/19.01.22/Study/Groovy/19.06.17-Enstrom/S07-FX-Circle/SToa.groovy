...
gp_request_exit = { x_ec, x_ex_list = [] -> gp_xr { __gap_request_exit x_ec, x_ex_list } }
...
class WAtMain {
  final wu_root = new javafx.scene.Group ()
  WAtMain () {
    GC_LOG .info gf_wai ()
    wn_init ()
  }
  def wn_init () {
    GC_LOG .info gf_wai ()
    new javafx.scene.shape.Circle () .with {
      centerX = 100
      centerY = 100
      radius = 50
      fill = javafx.scene.paint.Color.BLUE
      wu_root.children .addAll it
    }
    GC_PS .with {
      onShown = { bx_ev -> GC_LOG .info 'Primary stage shown' }
      ...
      width = 500
      height = 300
      scene = new javafx.scene.Scene (wu_root)
      show ()
      toFront ()
    }
  }
  ...
  void receive (x_letter) {
  }
}

Object.metaClass.sp_body = {
  pu_atr_m = gf_mk_atr_jo ( new WAtMain (), [], ':c' )
}
