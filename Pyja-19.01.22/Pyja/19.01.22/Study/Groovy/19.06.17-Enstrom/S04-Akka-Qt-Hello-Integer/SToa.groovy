...
class WAtMain extends QMainWindow {
  final wu_atr_integer
  final wu_pb = new QPushButton ()
  WAtMain (x_atr_integer) { wu_atr_integer = x_atr_integer }
  def preStart () { gp_qr { wn_init () } }
  def receive (x_letter) { gp_qr { wn_set_pb_text x_letter } }
  def postStop () {}
  def wn_init () {
    GC_LOG .info gf_wai ( 'Entering ...' )
    windowTitle = GC_TONO_NM
  ...
  def wn_set_pb_text (x_cnt) { wu_pb .setText gf_sf ( "Ciao, %,d !", x_cnt ) }
  def wn_move_center () {
  ...
  void showEvent ( QShowEvent x_ev ) {
    GC_LOG .info gf_wai ()
    new Timer () .schedule ( { gp_qr { wu_pb .click () } } as TimerTask, 0, 1 )
  }
...
Object.metaClass.sp_body = {
  ...
  gf_mk_atr_jo new WAtMain (pu2_atr_integer), ':c'
