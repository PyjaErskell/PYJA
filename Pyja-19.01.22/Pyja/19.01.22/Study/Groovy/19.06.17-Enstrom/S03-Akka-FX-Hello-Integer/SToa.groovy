...
class __CgFxApp extends javafx.application.Application {
  ...
  void start ( final javafx.stage.Stage x_stage ) { gp_xr { __casp_start x_stage } }
}
...
class WAtMain {
  final wl_atr_integer
  def wl_ps
  def wl_bn
  def wv_cnt = Integer.MAX_VALUE - 20
  WAtMain (x_atr_integer) {
    wl_atr_integer = x_atr_integer
    gp_fx_start this .&wn_start
  }
  void wn_start ( final Stage x_stage ) {
    GC_LOG .info gf_wai ()
    wl_ps = x_stage
    wl_bn = new Button ()
    wl_bn .with {
      wl_atr_integer .tell 'LCurrent', getSelf ()
      onAction = { bx_ev ->
        GC_LOG .info text
        wl_atr_integer .tell 'LNext', getSelf ()
      }
    ...
    final nu_root = new VBox () .tap {
      ...
      spacing = 10
      children .addAll wl_bn
    }
    wl_ps .with {
      onShown = { bx_ev ->
        new Timer () .schedule ( { gp_xr { wl_bn .fire () } } as TimerTask, 0, 1 )
      }
  ...
  def receive (x_letter) { gp_xr { wn_set_pb_text x_letter } }
  def postStop () {}
  def wn_set_pb_text (x_cnt) { wl_bn.text = gf_sf 'Ciao, %,d !', x_cnt }
  def wn_move_center () {
...
Object.metaClass.sp_body = {
  // 1 / 0
  final pu_atr_integer = gf_mk_atr_jo new CAtInteger ( Integer.MAX_VALUE - 20 ), ':c'
  final pu_atr_main = gf_mk_atr_jo new WAtMain (pu_atr_integer), ':c'
}
