...
class __CgFxApp extends javafx.application.Application {
  ...
  static void csn_launch ( final Closure xp_start ) {
    __casp_start = xp_start
    launch this
  }
  void start ( final javafx.stage.Stage x_stage ) {
    Object.metaClass.GC_PS = x_stage
    gp_xr { __casp_start () }
  }
}
gp_fx_start = { xp_start -> Thread .start { __CgFxApp .csn_launch xp_start } }
...
class CAtInteger {
  def cv_value
  CAtInteger (x_value) { cv_value = x_value }
  def preStart () {
    GC_LOG .info gf_wai ()
  }
...
class WAtFxMain {
  final wu_atr_integer
  def wl_bt
  WAtFxMain (x_atr_integer) {
  ...
  def receive (x_letter) { gp_xr { wn_set_bt_text x_letter } }
  def postStop () {}
  def wn_init () {
    GC_LOG .info gf_wai ()
    wl_bt = new Button ()
    ...
    final nu_root = new VBox () .tap {
      ...
      spacing = 10
      children .addAll wl_bt
    }
    GC_PS .with {
      onShown = { bx_ev ->
        context.system.scheduler .schedule CgDuration .Zero (), CgDuration.create ( 5, CgTimeUnit.MILLISECONDS ), wu_atr_integer, 'LNext', context.system.dispatcher, self
      }
      onCloseRequest = { bx_ev ->
        GC_LOG .info 'About to quit ...'
        gp_request_exit GC_EC_SUCCESS
      }
      title = gf_sf "[FX] %s", GC_TONO_NM
      width = 350
      height = 150
      scene = new Scene (nu_root)
      wn_move_above_center ()
      show ()
      toFront ()
    }
  }
  def wn_set_bt_text (x_cnt) {
    wl_bt.text = gf_sf 'Ciao, %,d !', x_cnt
    GC_LOG .info gf_wai (wl_bt.text)
  }
  def wn_move_above_center () {
...
class WAtQtMain extends QMainWindow {
  final wu_atr_integer
  final wu_pb = new QPushButton ()
  WAtQtMain (x_atr_integer) { wu_atr_integer = x_atr_integer }
  def preStart () {
    GC_LOG .info gf_wai ()
    gp_qr { wn_init () } 
  }
  def receive (x_letter) { gp_qr { wn_set_pb_text x_letter } }
  def postStop () {}
  def wn_init () {
    GC_LOG .info gf_wai ( 'Entering ...' )
    windowTitle = gf_sf "[Qt] %s", GC_TONO_NM
    final nu_cw = new QWidget ()
    final nu_lo = new QVBoxLayout ()
    wu_pb .with {
      wu_atr_integer .tell 'LCurrent', getSelf ()
      setSizePolicy QSizePolicy.Policy.Expanding, QSizePolicy.Policy.Fixed
      pressed .connect this, 'won_pb_pressed()'
    }

    nu_lo .addWidget wu_pb
    nu_cw.layout = nu_lo
    setCentralWidget nu_cw
    resize 350, 150
    wn_move_below_center ()
    show ()
    raise ()
  ...
  def wn_set_pb_text (x_cnt) {
    wu_pb .setText gf_sf ( "Ciao, %,d !", x_cnt )
    GC_LOG .info gf_wai ( wu_pb .text () )
  }
  def wn_move_below_center () {
    final nu_cp = new QDesktopWidget () .availableGeometry () .center () // center point
    move nu_cp .x () - (Integer) ( width () / 2 ), nu_cp .y ()
  }
...
Object.metaClass.sp_body = {
  ...
  final pu2_atr_integer = gf_mk_atr_jo new CAtInteger ( Integer.MAX_VALUE - 20 ), ':c'
  gf_mk_atr_jo new WAtQtMain (pu2_atr_integer), ':c'
  gf_mk_atr_jo new WAtFxMain (pu2_atr_integer), ':c'
}
