...
gp_os_exit = { x_ec -> Runtime .getRuntime () .halt (x_ec) }
...
gf_method = { x_jo, String x_yethod_nm, x_wai -> // get java object's method
  if ( ! gf_respond_to ( x_jo, x_yethod_nm ) ) {
    gp_request_exit GC_EC_ERROR, [ "Can't find yethod $x_yethod_nm of object $x_jo", x_wai ]
  }
  x_jo .&"$x_yethod_nm"
}

GC_Q4ATR = [] as java.util.concurrent.LinkedBlockingQueue // (Q)ueue (4)for (A)kka (T)ell (R)uby object
gp_akka_tell_ro = { long x_ro, String x_nethod_mn, Object ... x_args -> GC_Q4ATR .put ( [ x_ro, x_nethod_mn, x_args ] as Object [] ) }

CgAtJo = Eval .me '''
  class CgAtJo extends akka.actor.AbstractActor {
    final private Object __cau_jo_at
    CgAtJo (x_jo_at) { __cau_jo_at = x_jo_at }
    void preStart () { gy_call_if_exist __cau_jo_at, 'preStart', getSelf (), getContext () }
    akka.actor.AbstractActor.Receive createReceive () {
      return receiveBuilder () .match ( Object.class, { x_letter ->
        gy_call_if_exist __cau_jo_at, 'receive', x_letter, getSender ()
      } ) .build ()
    }
...
'''
CgAtRo = Eval .me '''
  class CgAtRo extends akka.actor.AbstractActor {
    final private long __cau_ro_at
    CgAtRo (x_qo_at) { __cau_ro_at = x_qo_at }
    void preStart () { gp_akka_tell_ro __cau_ro_at, 'preStart', getSelf (), getContext () }
    akka.actor.AbstractActor.Receive createReceive () {
      return receiveBuilder () .match ( Object.class, { x_letter ->
        gp_akka_tell_ro __cau_ro_at, 'receive', x_letter, getSender ()
      } ) .build ()
    }
    void postStop () { gp_akka_tell_ro __cau_ro_at, 'postStop' }
  }
  CgAtRo
'''
...
'''
gf_mk_atr_ro = Eval .me '''
  { long x_at_ro, String x_at_nm, akka.actor.ActorRefFactory x_arf = GC_AS ->
    switch (x_at_nm) {
      case ':a' : return x_arf .actorOf ( akka.actor.Props .create ( CgAtRo, x_at_ro ) )
      default : return x_arf .actorOf ( akka.actor.Props .create ( CgAtRo, x_at_ro ), x_at_nm )
    }
  }
'''
