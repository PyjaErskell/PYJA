...
$ju_platform = jf_cls 'com.sun.jna.Platform'
...
if $ju_platform .isMac
  jy_ge <<-BASH2_EOS.unindent
  ...
  jy_ge <<-BASH2_EOS.unindent
    com.trolltech.qt.gui.QApplication .initialize ()
    Object.metaClass.gp_qr = { final Closure xp_it -> com.trolltech.qt.core.QCoreApplication.invokeLater { xp_it () } } // qr = (Q)tJambi (r)un
    Object.metaClass.gy_ry = { final long x_ri, final String x_yethod_nm, final Object... x_args ->
      gp_qr { GC_RUBY .cy_call ( x_ri, x_yethod_nm, *x_args ) }
    }
  BASH2_EOS
end
...
def jp_xr x_args, x_arg_nms_str, x_script
  pu_closure_str = """
    { #{x_arg_nms_str} -> gp_xr {
      #{x_script}
    }}
  """
  jy_gc pu_closure_str, x_args
end
...
private
  def self.__dap_begin
    ...
    GC_LOG .info  "Is 64 bit? => #{ $ju_platform .is64Bit }"
...
class WMain < Gtk::Window
  ...
  def __wan_init
    ...
    @wu_rbnf = nu_cls_rbnf .new nu_cls_rbnf.SPELLOUT
    @wv_no = 10305009
    set_title GC_TONO_NM
    ...
    @wu_bn = Gtk::Button .new
    @wu_lb = Gtk::Label .new
    __wan_display_result
    @wu_lo = ( Gtk::Box .new :vertical, 0 ) .tap { |bx2_lo|
      bx2_lo .pack_start ( Gtk::Alignment .new 0, 0, 0, 0 ), :expand => true
      bx2_lo .add @wu_bn
      bx2_lo .pack_start ( Gtk::Alignment .new 0, 0, 0, 0 ), :expand => true
      bx2_lo .add @wu_lb
      bx2_lo .pack_start ( Gtk::Alignment .new 0, 0, 0, 0 ), :expand => true
    ...
    add @wu_lo
    @wu_bn .signal_connect 'clicked' do |it|
      @wv_no += Random .new .rand 5..100000000
      __wan_display_result
    end
    show_all
  end
  def __wan_display_result
    @wu_bn .set_label "Number : #{ CjString .format '%,d', [@wv_no] }"
    @wu_lb .set_label "Spellout : #{ @wu_rbnf .format @wv_no }"
  end
end
