...
class WMain < CgRi
  ...
  def wn_fx_start x_stage; gy_rr {
    ...
    @wu_stage .tap { |it|
      ...
      jp_xr [ it, @wu_root ], 'x_stage, x_root', <<-NASH_EOS.unindent
        x_stage .with {
          scene = new javafx.scene.Scene (x_root)
          centerOnScreen ()
          show ()
          toFront ()
        }
  ...
  def wn_change_font
    ...
    jp_xr [@wu_bn], 'x_bn', <<-NASH_EOS.unindent
      x_bn.text = "Say '#{@wv_msg}'"
      x_bn.style = "-fx-font-family : '#{nu_fnt_nm}'; -fx-font-size : 27px;"
    NASH_EOS
    jp_xr [@wu_lb], 'x_lb', "x_lb.text = '#{nu_nt} Font name : #{nu_fnt_nm}'"
    @wv_fnt_idx += 1
  end
