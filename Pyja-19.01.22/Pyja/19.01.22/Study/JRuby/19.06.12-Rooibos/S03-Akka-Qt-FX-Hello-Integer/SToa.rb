...
def gf_mk_atr_ro x_at_ro, x_args, x_at_nm, x_arf = GC_AS
  ff2_props = -> { Java::AkkaActor::Props .create CgAt.java_class, ( $__CgAkCreator .new x_at_ro, x_args ) }
  case x_at_nm
...
class WAtQtMain < QMainWindow
  ...
  def showEvent x_ev
    ...
    getContext.system.scheduler .schedule CgDuration::Zero(), CgDuration.create( 15, CgTimeUnit::MILLISECONDS ), @wu_atr_integer, :LNext, getContext.system.dispatcher, getSelf
  end
  def closeEvent x_ev
...
class WAtFxMain
  ...
  def wn_shown x_ev
    GC_LOG .info gf_wai
    @wu_atr_integer .tell :LCurrent, getSelf
    getContext.system.scheduler .schedule CgDuration::Zero(), CgDuration.create( 10, CgTimeUnit::MILLISECONDS ), @wu_atr_integer, :LNext, getContext.system.dispatcher, getSelf
  end
  def wn_set_pb_text x_cnt
    GC_LOG .info "[FX] #{@wu_bn.text}"
