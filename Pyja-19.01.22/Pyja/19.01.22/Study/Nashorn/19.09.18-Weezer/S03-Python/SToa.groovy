...
gf_new_python = { ->
  ...
  fu_disp .call fu_rbo, 'erubyize', """
    ...
    -> () {
      ...
      gp_set_os_env :PYTHONPATH, ( [
        '',
        pu_python_hm,
        ( gf_pj pu_python_hm, 'DLLs' ) ,
        ( gf_pj pu_python_hm, 'lib' ),
        ( gf_pj pu_python_hm, 'lib', 'site-packages' ),
        ( gf_pj pu_python_hm, 'lib', 'site-packages', 'Pythonwin' ),
        ( gf_pj pu_python_hm, 'lib', 'site-packages', 'win32' ),
        ( gf_pj pu_python_hm, 'lib', 'site-packages', 'win32', 'lib' ),
        ( gf_pj pu_python_hm, 'python36.zip' ),
      ] .join GC_PASA )
...
println "Milo path ($GC_MILO_PN_SYM) => $GC_MILO_PN"
println "Tono home ($GC_TONO_HM_SYM) => ${ gf_to_mps GC_TONO_HM }"
println "Kapa home ($GC_KAPA_HM_SYM) => $GC_KAPA_HM"
