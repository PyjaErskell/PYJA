...
GC_PYJA_CD = '{:02d}{:02d}.{:02d}.{:02d}' .format ( GC_PJYA_CEN, GC_PYJA_YEA, GC_PYJA_MON, GC_PYJA_DAY ) # Pyja creation date
GC_PYJA_VR = '{:02d}.{:02d}.{:02d}' .format ( GC_PYJA_YEA, GC_PYJA_MON, GC_PYJA_DAY )                    # Pyja version with fixed length 8
GC_PYJA_V2 = '{}.{}.{}' .format ( GC_PYJA_YEA, GC_PYJA_MON, GC_PYJA_DAY )                                # Pyja version without leading zero
...
def gf_on (x_it) : # f(o)lder (n)ame
  fu_pn = gf_pn (x_it) if gf_if (x_it) else gf_ap (x_it)
...
def gp_log_exception ( xp_out, x_title, x_ex, x_header_line_width ) :
  ...
  if pu_type is str : 
    ...
    gp_log_header ( xp_out, 'Invalid exception type !!!', x_header_line_width )
    xp_out ( '{}' .format (x_ex) )
...
class __DgLongLiveObjects :
  ...
  def df_cllo ( cls, x_it, *x_args ) : # cllo -> (c)reate (l)ong (l)ive (o)bject
    ...
    if inspect.isclass (x_it) :
      ...
      raise Exception ( 'Unsupported {} !!!' .format (x_it) )
    fu_yi = gf_yi (fu_yo)
...
def gf_wai ( x_msg = None ) :
  ...
  fu_msg = '@ {}.{} [{:03}]' .format ( fu_module_nm, fu_func_nm, fu_lineno )
...
class TgWai : # (T)rait (g)lobal 
  ...
  def __tasm_wai ( x_qualname, x_msg = None ) : # tasm -> (t)rait priv(a)te (s)tatic (m)ethod
    ...
    mu_msg = '@ {}.{}.{} [{:03}]' .format ( mu_module_nm, x_qualname, mu_func_nm, mu_lineno )
...
jy_ge ('''
  ...
  GC_EC_ERROR    = {}
  GC_THIS_PID    = {}
''' .format ( GC_APP_NM, GC_EC_NONE, GC_EC_SHUTDOWN, GC_EC_SUCCESS, GC_EC_ERROR, GC_THIS_PID ) )
jy_ge ('''
...
def jf_jarray (x_py_list) :
  ...
  fu_params = jy_ge ( 'new Object [{}]' .format (fu_arr_sz) )
...
def jf_gi (x_cls) : return jy_ge ( '@groovy.transform.Immutable {}' .format (x_cls) ) # (g)roovy (i)mmutable class
...
class __DjExit : # (__)private mo(D)ule (g)lobal
  ...
  @classmethod
  def dp_it ( cls, x_cx ) :
    JC_LOG .debug ( 'Received LgCx ({})' .format (x_cx.lu_ec) )
  ...
  def __dap_before_exit (cls) :
    ...
    elif pu_ec < 0 : JC_LOG .error ( 'Negative exit code ({}), should consider using a positive value !!!' .formt (pu_ec) )
...
def __jap_exit ( x_ec, x_ex = None ) :
  pu_llos = gf_llos ()
  if len(pu_llos) > 0 :
    JC_LOG .warn ( 'You did not delete {} llo(s) with gp_dllo () function after using llo(s) !!!' .format (len(pu_llos)) )
...
def jp_request_exit ( x_ec, x_ex_list = None ) :
  ...
  if pu_ex == None : jy_ge ( "gp_xr {{ gp_cy ( '__jap_exit', {} ) }}" .format (x_ec) )
  else :
    jy_gc ('''
...
class TjFx :
  def tn_fx_set_eh ( self, x_xo, x_xo_eh_nm, x_yo_nethod_nm ) : # xo -> F(X) (o)bject, eh -> fx (e)vent (h)andler
    jy_gc ( """{{ x_xo ->
      x_xo.{} = {{ x2_ev -> gp_yn {}, '{}', x2_ev }}
    }}""" .format ( x_xo_eh_nm, self.cu_yi, x_yo_nethod_nm ) , x_xo  )
  def tn_fx_add_cl ( self, x_property, x_yo_nethod_nm ) : # cl -> (c)hange (l)istener
    jy_gc ( """{{ x_property ->
      x_property .addListener ( {{ x2_obs, x2_old, x2_new -> gp_yn {}, '{}', x2_obs, x2_old, x2_new }} as javafx.beans.value.ChangeListener )
    }}""" .format ( self.cu_yi, x_yo_nethod_nm ) , x_property  )
...
class DRun :
  ...
  def __dap_begin (cls) :
    print ( "\n" .join ( gf_banner () ) )
    JC_LOG .debug ( 'Pyja name => {}' .format (GC_PYJA_NM) )
    if GC_PYJA_NM != gf_os_env ('SC_PYJA_NM') : raise Exception ( 'Invalid Pyja name !!!' )
    JC_LOG .debug ( 'Pyja creation date => {}' .format (GC_PYJA_CD) )
    if not gf_str_is_valid_date ( GC_PYJA_CD, '%Y.%m.%d' ) : raise Exception ( 'Pyja create date is not invalid !!!' )
    JC_LOG .debug ( 'Pyja version => {}' .format (GC_PYJA_V2) )
    if GC_PYJA_VR != gf_os_env ('SC_PYJA_VR') : raise Exception ( 'Invalid Pyja version !!!' )
    JC_LOG .info  ( 'Pyja root ({}) => {}' .format ( GC_PYJA_RT_SYM, GC_PYJA_RT ) )
    JC_LOG .info  ( 'Pyja home ({}) => {}' .format ( GC_PYJA_HM_SYM, gf_to_prs (GC_PYJA_HM) ) )
    JC_LOG .info  ( 'Milo path ({}) => {}' .format ( GC_MILO_PN_SYM, gf_to_phs (GC_MILO_PN) ) )
    JC_LOG .info  ( 'Java version => {}' .format (JC_JAVA_VR) )
    JC_LOG .info  ( 'Python version => {}' .format (GC_PYTHON_VR) )
    JC_LOG .info  ( 'Tcl version => {}' .format (GC_TCL_VR) )
    JC_LOG .info  ( 'PyQt version => {}' .format (GC_PYQT_VR) )
    JC_LOG .info  ( 'Groovy version => {}' .format (JC_GROOVY_VR) )
    JC_LOG .debug ( 'Python home => {}' .format (GC_PYTHON_HM) )
    JC_LOG .debug ( 'Total CPU => {}' .format (GC_TOTAL_CPU) )
    JC_LOG .debug ( 'Total memory => {:,d} bytes' .format (GC_TOTAL_MEMORY) )
    JC_LOG .debug ( 'Available memory => {:,d} bytes' .format (gf_os_available_memory()) )
    JC_LOG .debug ( 'Computer name => {}' .format (GC_HOST_NM) )
    JC_LOG .debug ( 'Current user => {}' .format (GC_CUSR) )
    JC_LOG .debug ( 'Process ID => {}' .format (GC_THIS_PID) )
    JC_LOG .debug ( 'Executable file => {}' .format (GC_THIS_EXE_FN) )
    JC_LOG .info  ( 'Start up path => {}' .format( gf_to_phs (GC_THIS_START_UP_PN) ) )
    JC_LOG .info  ( 'Script file => {}' .format ( gf_to_mps (GC_SCRIPT_FN) ) )
...
class HSpellout (QAbstractTableModel) :
  def __init__ ( self, x_total ) :
    self .wu_total = x_total
    super () .__init__ ()
    self .wn_init ()
  def wn_init (self) :
    nu_cls_rbnf = jf_jcls ('com.ibm.icu.text.RuleBasedNumberFormat')
    self.wu_rbnf = nu_cls_rbnf ( nu_cls_rbnf.SPELLOUT )
    self.wu_fixed_fnt = QFontDatabase.systemFont (QFontDatabase.FixedFont)
  def rowCount ( self, x_parent = QModelIndex () ) : return self .wu_total
  def columnCount ( self, x_parent = QModelIndex () ) : return 2
  def headerData ( self, x_column, x_orientation, x_role ) :
    if x_role == Qt.DisplayRole :
      if x_orientation == Qt.Horizontal :
        if x_column == 0 : return QVariant ('Number')
        elif x_column == 1 : return QVariant ('Spell out')
    return QVariant ()
  def data ( self, x_index, x_role ) :
    if not x_index .isValid () : return QVariant ()
    elif x_role == Qt.DisplayRole :
      bu2_col = x_index .column ()
      bu2_row = x_index .row ()
      if bu2_col == 0 : return QVariant ( '{:,d}'.format (bu2_row) )
      elif bu2_col == 1 : return QVariant ( self.wu_rbnf .format (bu2_row) )
    elif x_role == Qt.TextAlignmentRole :
      bu2_col = x_index .column ()
      if bu2_col == 0 : return Qt.AlignRight | Qt.AlignVCenter
      elif bu2_col == 1 : return Qt.AlignLeft | Qt.AlignVCenter
    elif x_role == Qt.FontRole :
      bu2_col = x_index .column ()
      if bu2_col == 0 : return self.wu_fixed_fnt
...
class WAtQtMain ( CjAt, TgWai, TjFx ) :
  ...
  def wn_init_ui (self) :
    ...
    def nf2_qw_layout () :
      ...
      def ff3_qw_tv () :
        fu3_tv = QTableView ()
        fu3_tv .verticalHeader () .hide ()
        fu3_tv .horizontalHeader () .setStretchLastSection (True)
        fu3_tv .setSelectionBehavior (QAbstractItemView.SelectRows)
        fu3_tv .setSelectionMode (QAbstractItemView.SingleSelection)
        fu3_tv .keyboardSearch = lambda x4_search : None
        fu3_tv .setModel ( HSpellout (8388605) )
        fu3_tv .scrollToBottom ()
        return fu3_tv
      self.wu_qw_tv = ff3_qw_tv ()
      fu2_lo .addWidget (self.wu_qw_tv)
      return fu2_lo
    self.wu_qw = nf2_qw ()
