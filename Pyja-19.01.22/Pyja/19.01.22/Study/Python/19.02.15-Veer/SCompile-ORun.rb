...
GC_JRUBY_HM = gf_os_env(:SC_JRUBY_HM)
...
module DBody
  def self.dp_it
    ...
    @__dau_java_src_fn = gf_pj GC_MILO_PN, "#{GC_APP_NM.split('-')[-1]}.java"
    GC_LOG .info "Java source file => #{ gf_to_mps @__dau_java_src_fn }"
...
private
  def self.__dap_compile_java
    ...
    system pu_javac_x_fn, '-cp', gf_os_env(:SC_JEP_JAR_FN), '-d', @__dau_cls_o_pn, @__dau_java_src_fn
    raise RuntimeError, 'Error during compiling !!!' unless $?.exitstatus == 0
  end
  def self.__dap_make_jar
    gp_log_header 'Making jar file ...', 40
    @__dau_ecu_pn = gf_pj GC_MILO_PN, 'ecu'
    gp_mp @__dau_ecu_pn
    @__dau_out_jar_fn = gf_pj @__dau_ecu_pn, "#{@__dau_src_jn}.jar"
