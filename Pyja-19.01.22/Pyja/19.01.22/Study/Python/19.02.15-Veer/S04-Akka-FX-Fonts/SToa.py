...
XScreen = jf_jcls ('javafx.stage.Screen')
XVBox   = jf_jcls ('javafx.scene.layout.VBox')

class WAtFxMain ( CjAt, TgWai, TjFx ) :
  ...
  def wn_init (self) :
    ...
    self.wu_stage .show ()
  def timerEvent ( self, x_ev ) : self .tell ( self .getSelf (), CjSymbol ('LNextFont') )
  def receive ( self, x_letter, x_atr_sender ) :
    if jf_is_instance ( CjSymbol, x_letter ) :
      bu2_nm = x_letter .name ()
      JC_LOG .debug ( self .tm_wai ( 'Received symbol {}' .format (bu2_nm) ) )
  ...
  def wn_change_font (self) :
    ...
    if self.wv_msg != '' : JC_LOG .info ( '[FX] {}' .format (self.wv_msg) )
    nu_nt = '({}/{})' .format ( self.wv_fnt_idx + 1, nu_total )
    self.wv_msg = '0^0 {} {}' .format ( nu_nt, nu_fnt_nm )
    self.wu_bn .setText ( "Say '{}'" .format (self.wv_msg) )
    self.wu_bn .setStyle ( "-fx-font-family : '{}'; -fx-font-size : 17px;" .format (nu_fnt_nm) )
    self.wu_lb .setText ( '{} Font name : {}' .format ( nu_nt, nu_fnt_nm ) )
    self .wn_center_on_screen ()
    self.wv_fnt_idx += 1
  def wn_center_on_screen (self) :
    nu_vb = XScreen .getPrimary () .getVisualBounds ()
    self.wu_stage .setX ( nu_vb .getWidth () / 2 - self.wu_stage .getWidth () / 2 )
    self.wu_stage .setY ( nu_vb .getHeight () / 2 - self.wu_stage .getHeight () / 2 )
