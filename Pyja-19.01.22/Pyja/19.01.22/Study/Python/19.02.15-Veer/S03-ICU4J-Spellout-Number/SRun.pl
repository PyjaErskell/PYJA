...
package DBody {
  ...
  sub dp_it {
    gp_set_os_env 'JAVA_HOME', gf_os_env 'SC_J11_HM';
    my $pu_java_xmx = '-Xmx9g';
    ...
    my @pu_all_jars = (
      ...
      gf_pj ( GC_KAPA_HM, '19.01.22', 'Cumuni', 'ICU4J', '63.1', 'icu4j-63_1.jar' ),
      gf_pj ( GC_KAPA_HM, '19.01.22', 'Cumuni', 'ICU4J', '63.1', 'icu4j-charset-63_1.jar' ),
      gf_pj ( GC_KAPA_HM, '19.01.22', 'Cumuni', 'ICU4J', '63.1', 'icu4j-localespi-63_1.jar' ),
    );
    ...
    my @pu_all_java_module_paths = (
      gf_pj ( gf_os_env ('SC_FX11_HM'), 'lib' ),
    );
    gp_log_array sub { GC_LOG -> info (@_) }, 'Java module paths', map { gf_to_kms $_ } @pu_all_java_module_paths;
    my @pu_all_java_modules = (
      'javafx.controls',
    );
    gp_log_array sub { GC_LOG -> info (@_) }, 'Java modules', @pu_all_java_modules;
    my $pu_rc = system (
      gf_os_env ('SC_J11_X_FN'),
      $pu_java_xmx,
      '--module-path', join ( GC_PASA, @pu_all_java_module_paths ),
      "--add-modules=${\ join ( ',', @pu_all_java_modules ) }",
