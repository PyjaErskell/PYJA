...
class WAtQtMain ( CjAt, TgWai ) :
  ...
  def wn_init_ui (self) :
    ...
    def nf2_qw_layout () :
      ...
      def ff3_add_qw_config (x3_row) :
        ...
        self.wu_qw_c_flip_vertically_cb = QCheckBox ('Flip vertically')
        self.wu_qw_c_lo .addWidget (self.wu_qw_c_flip_vertically_cb)
  ...
  def won_qw_p_do_pb_clicked ( self, x_checked ) :
    ...
    def nf2_flip_box ( x2_horizontally, x2_mb, x2_bx ) :
      if x2_horizontally : return CjRectangle ( x2_mb .getRight () - ( x2_bx .getX () - x2_mb .getX () + x2_bx .getWidth () ), x2_bx .getY (), x2_bx .getWidth (), x2_bx .getHeight () )
      else : return CjRectangle ( x2_bx .getX (), x2_mb .getTop () - ( x2_bx .getY () - x2_mb .getY () + x2_bx .getHeight () ), x2_bx .getWidth (), x2_bx .getHeight () )
    def np2_do () :
      ...
      try :
        ...
        try :
          ...
          bu4_flip_horizantally = not self.wu_qw_c_flip_vertically_cb .isChecked ()
          if bu4_flip_horizantally : self .wn_log_info ('Flipping horizantally ...')
          else : self .wn_log_info ('Flipping vertically ...')
          bv4_o_nop_so_far = 0
          ...
          for bu5_i_pg_no in bu4_page_no_list :
            ...
            if bu4_flip_horizantally : CjPdfCanvas (bu5_o_pg) .addXObject ( bu5_pg_cp, -1, 0, 0, 1, bu5_i_pg_mb .getWidth (), 0 )
            else : CjPdfCanvas (bu5_o_pg) .addXObject ( bu5_pg_cp, 1, 0, 0, -1, 0, bu5_i_pg_mb .getHeight () )
            bu5_o_pg .setMediaBox (bu5_i_pg_mb)
            bu5_o_pg .setBleedBox ( nf2_flip_box ( bu4_flip_horizantally, bu5_i_pg_mb, bu5_i_pg_bb ) )
            bu5_o_pg .setTrimBox ( nf2_flip_box ( bu4_flip_horizantally, bu5_i_pg_mb, bu5_i_pg_tb ) )
            bu5_o_pg .setCropBox ( nf2_flip_box ( bu4_flip_horizantally, bu5_i_pg_mb, bu5_i_pg_cb ) )
            bv4_o_nop_so_far += 1
