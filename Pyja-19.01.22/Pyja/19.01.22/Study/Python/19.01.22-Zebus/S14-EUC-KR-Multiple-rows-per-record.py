...
class WAtQtMain ( CjAt, TgWai ) :
    ...
    def nf2_qw_layout () :
      ...
      def ff3_add_qw_config (x3_row) :
        ...
        def ff4_qw_sb () :
          ...
          fu4_sb .setPrefix ( 'repeat the record ' )
  ...
  def won_qw_i_fn_pb_clicked ( self, x_checked ) :
    def np2_do () :
      pu2_i_fn = QFileDialog .getOpenFileName ( self.wu_qw, 'Select text file', self.wu_qw_i_pn_le .text (), 'Text file (*.txt)' ) [0]
      if not gf_if (pu2_i_fn) : return
      self .wn_log_clear ()
      self .wn_log_info ( f'Requested file => {pu2_i_fn}' )
      self .wn_log_info ( 'Getting total fields of the requested file ...' )
      self.wu_qw_i_pn_le .setText ( gf_pn (pu2_i_fn) )
      self.wv_i_nof = self .wm_txt_nof (pu2_i_fn)
      self.wu_qw_i_bn_le .setText ( gf_bn (pu2_i_fn) )
      self .wn_log_info ( f"Total fields of the requested file => {self.wv_i_nof:,d}" )
      self.wu_qw_i_nfo_le .setText ( f'{self.wv_i_nof:,d} field(s)' )
      self .wn_c_changed ()
      self.wv_i_recent_pn = gf_pn (pu2_i_fn)
    try : np2_do ()
    except :
      gp_log_exception ( self.wn_log_error, 'Following error occurs !!!', gf_exception_to_list (), 30 )
      self.wn_log_error ( 'Check your input text file or this program logic !!!' )
  def won_qw_o_fn_pb_clicked ( self, x_checked ) :
    if self.wv_o_recent_pn == None : self.wv_o_recent_pn = self.wv_i_recent_pn
    nu_o_fn, _ =  QFileDialog .getSaveFileName ( self.wu_qw, 'Specify output text file', self.wv_o_recent_pn, 'Text file (*.txt)' )
    if nu_o_fn == '' : return
  ...
  def won_qw_p_do_pb_clicked ( self, x_checked ) :
    ...
    def np2_do () :
      ...
      try :
        ...
        bu3_i_fl = open ( bu3_i_fn, 'r', encoding = "euc-kr" )
        self .wn_log_info ( 'Opening output file ...')
        bu3_o_fl = open ( nu_o_fn, 'w', encoding = "utf-16" )
        JC_LOG .info ( 'Initializing progress bar ...' )
        bu3_i_nof = self.wv_i_nof
        bu3_repeat = self.wu_qw_c_repeat_sb .value ()

        self .wn_log_info ( 'Generating output file ( may take a long time ) ...')
        bu3_i_fns = bu3_i_fl .readline () .rstrip () .split ("\t")
        bu3_o_fns = []
        for bu4_i in range (bu3_repeat) :
          bu4_r = bu4_i + 1
          for bu5_f in bu3_i_fns :
            bu3_o_fns .append (f'{bu5_f}#{bu4_r}')
        bu3_o_first_line = '\t' .join (bu3_o_fns)
        self .wn_log_info ( f'Output\'s first line => {bu3_o_first_line}' )
        bu3_o_fl .write (f'{bu3_o_first_line}\n')
        ...
        try :
          ...
          while True :
            bv5_vals = []
            for bu6_i in range (bu3_repeat) :
              bu6_line = bu3_i_fl .readline ()
              if not bu6_line :
                bv4_is_eof = True
                break
              bv4_i_nor_so_far += 1
              bu6_vals = bu6_line .rstrip () .split ("\t")
              if bu3_i_nof != len (bu6_vals) : 
                raise Exception ( f'Input line {bv4_i_nor_so_far} has not {bu3_i_nof} fields !!!' )
              bv5_vals .extend (bu6_vals)
            bv5_vals = bv5_vals + [''] * ( bu3_i_nof * bu3_repeat - len (bv5_vals) )
            # self .wn_log_info ( f'bv5_vals => {bv5_vals}')
            bu5_o_line = '\t' .join (bv5_vals)
            bu3_o_fl .write (f'{bu5_o_line}\n')
            bv4_o_nor_so_far += 1
            if bv4_is_eof : break
          self .wn_log_info ( f'Total input records => {bv4_i_nor_so_far:,d}' )
          self .wn_log_info ( f'Total output records => {bv4_o_nor_so_far:,d}' )
        finally :
          bu3_o_fl .close ()
          bu3_i_fl .close ()
    ...
    try : np2_do ()
      ...
      gp_log_exception ( self.wn_log_error, 'Following error occurs !!!', gf_exception_to_list (), 30 )
      self.wn_log_error ( 'Check your input text file or this program logic !!!' )
  def wm_txt_nof ( self, x_fn ) : # (n)umber (o)f (f)ields
    with open ( x_fn, encoding = "euc-kr" ) as bu2_fl :
      mu_first_line = bu2_fl .readline () .rstrip ()
    self .wn_log_info ( f'Input\'s first line => {mu_first_line}')
    mu_nof = len ( mu_first_line .split ("\t") )
    self .wn_log_info ( f'Input\'s number of fields => {mu_nof}')
    return mu_nof
  def wn_c_changed (self) :
    self.wu_qw_c .setEnabled (True)
    self.wu_qw_o_fn_pb .setEnabled (True)
    self.wu_qw_p_do_pb .setEnabled (True)
    nu_o_nop = self.wu_qw_c_repeat_sb .value () * self.wv_i_nof
    self.wu_qw_o_nfo_le .setText ( f'{nu_o_nop:,d} fields' )
  def wn_quit (self) :
