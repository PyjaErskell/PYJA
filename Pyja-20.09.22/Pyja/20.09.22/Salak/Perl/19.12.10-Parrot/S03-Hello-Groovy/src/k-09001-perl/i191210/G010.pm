...
use Inline (
  ...
  class OYela {
    public static String OC_FOSA = File.separator; // (fo)lder (s)ep(a)rator
    public static String OC_PASA = File.pathSeparator; // (pa)th (s)ep(a)rator
    ...
    static class CArrayList {
      ...
      public CArrayList () { cu_al = new ArrayList <Object> (); }
      public ArrayList <Object> cm_it () { return cu_al; }
      ...
      public void cn_add ( Object x_it ) { cu_al .add (x_it); }
    }
  }
KASH_EOS
  STUDY => [ 
...
sub jf_sf { # java string format
  my ( $x_format, @x_args ) = @_;
  java::lang::String -> format ( $x_format, jf_al (@x_args) -> to_a () )
}
sub jp_add_jar (_) {
  my $x_jar_fn = shift;
...
our $ju_g = javax::script::ScriptEngineManager -> new () -> getEngineByName ('Groovy');
sub jf_ge (_) { $ju_g -> eval (shift) } # (g)roovy (e)val
sub jf_gf { # (g)roovy invoke (f)unction
  my ( $x_nm, @x_args ) = @_;
  return $ju_g -> invokeFunction ( $x_nm, jf_al (@x_args) -> to_a () );
}
sub jf_gm { # (g)roovy invoke (m)ethod
  my ( $x_oj, $x_nm, @x_args ) = @_;
  return $ju_g -> invokeMethod ( $x_oj, $x_nm, jf_al (@x_args) -> to_a () );
}
...
sub gp_run {
  ...
  if ($@) {
    $pv_ec = GC_EC_ERROR;
    gp_log_exception ( undef, "!!!Following error occurs !!!", Inline::Java::caught ('java.lang.Exception') ? $@ -> getMessage () : $@ );
  }
