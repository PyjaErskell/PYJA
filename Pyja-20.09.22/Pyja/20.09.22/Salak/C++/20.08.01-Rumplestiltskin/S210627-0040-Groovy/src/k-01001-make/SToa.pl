...
sub sp_body {
  ...
  my $pu_src_pn = gf_pj GC_MILO_PN, 'src';
  my $pu_cpp_src_pn = gf_pj $pu_src_pn, 'k-00701-ya';
  my $pu_src_fn = gf_pj $pu_cpp_src_pn, 'SToa.cpp';
  GC_LOG -> info ( "C++ source file => ${\ gf_to_mps $pu_src_fn }" );
  my $pu_src_jn = gf_jn $pu_src_fn;
  my $pu_pro_fn = gf_pj $su_tmp_pn, "$pu_src_jn.pro";
  my @pu_inc_paths = ( $pu_src_pn, $pu_cpp_src_pn, split GC_PASA, gf_os_env 'SC_INC_PATH' );
  my @pu_libs = split GC_PASA, gf_os_env 'SC_LIBS';
  ...
  write_file $pu_pro_fn, <<~PASH_EOS;
    ...
    ${\ join ( "\n", map { "LIBS += \"$_\"" } @pu_libs ) }
    ${\ join ( "\n", map { "VPATH += \"$_\"" } @pu_v_paths ) }
    ${\ join ( "\n", map { "SOURCES += \"$_\"" } @pu_sources ) }
    PASH_EOS
